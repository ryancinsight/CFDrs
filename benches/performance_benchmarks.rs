...
fn benchmark_flow_operations(c: &mut Criterion) {
    ...
    for size in [16, 32, 64].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("kinetic_energy", size),
            size,
            |b, _| {
                b.iter(|| {
                    black_box(FlowOperations::kinetic_energy(&flow_field.velocity))
                })
            },
        );
    }
    ...
    for size in [10, 50, 100, 200].iter() {
        ...
        group.bench_with_input(
            ...
            |bench, _| {
                ...
                bench.iter(|| {
                    black_box(solver.solve(&matrix, &b, None).unwrap())
                })
            },
        );
    }
...
fn benchmark_scaling(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("full_computation", size),
            &size,
            |b, _| {
                b.iter(|| {

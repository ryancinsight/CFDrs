...
fn create_benchmark_matrix(size: usize) -> CsrMatrix<f64> {
    ...
    CsrMatrix::from_parts(values, indices, indptr, size, size)
        .expect("invariant: benchmark matrix has valid CSR structure")
}

/// Create a test vector for benchmarking
fn create_test_vector(size: usize) -> Array1<f64> {
    Array1::from_shape_fn([size], |[i]| (i as f64).sin() + 0.1)
}

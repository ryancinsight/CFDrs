...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if has_perf_metadata {
        match metadata_system.channels[0].get_metadata::<PerformanceMetadata>() {
            ...
            None => {
                println!("   Warning: Performance metadata exists but could not be retrieved");
            }
        }
    }
    println!();
    ...
    plot_geometry(
        &standard_system,
        out.join("unified/standard_system.png").to_str().unwrap(),
    )?;
    println!(
        "   ✓ Standard system: {}",
        out.join("unified/standard_system.png").display()
    );

    plot_geometry(
        &metadata_system,
        out.join("unified/metadata_system.png").to_str().unwrap(),
    )?;
    println!(
        "   ✓ Metadata system: {}",
        out.join("unified/metadata_system.png").display()
    );
    println!();

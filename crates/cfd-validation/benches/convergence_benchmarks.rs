...
fn benchmark_gci_calculation(c: &mut Criterion) {
    ...
    let values = vec![(1.0, 1.01), (10.0, 10.1), (100.0, 100.5), (1000.0, 1005.0)];
    ...
    for (f_fine, f_coarse) in values {
        group.bench_with_input(
            ...
            &(f_fine, f_coarse),
            |b, &(f_fine, f_coarse)| {
                b.iter(|| black_box(gci.compute_fine(black_box(f_fine), black_box(f_coarse))));
            },
        );
    }
    ...
    for history_size in [10, 50, 100].iter() {
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                b.iter(|| black_box(monitor.check_status()));
            },
        );
    }

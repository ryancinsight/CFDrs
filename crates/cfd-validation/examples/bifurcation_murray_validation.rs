...
use cfd_1d::resistance::{FlowConditions, HagenPoiseuilleModel, ResistanceModel};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Apparent viscosity at 100 s⁻¹: {:.2} mPa·s",
        mu_apparent * 1e3
    );
    ...
    println!(
        "  P_inlet  = {:.2} Pa ({:.2} mmHg)",
        p_inlet_analytic,
        p_inlet_analytic / 133.322
    );
    println!(
        "  P_bifurc = {:.2} Pa ({:.2} mmHg)",
        p_bifurc_analytic,
        p_bifurc_analytic / 133.322
    );
    ...
    let p_bifurc = *solution
        .pressures
        .get(&node_idx["Bifurcation"])
        .unwrap_or(&0.0);
    ...
    let q_d1 = *solution
        .flow_rates
        .get(&edge_idx["Daughter1"])
        .unwrap_or(&0.0);
    let q_d2 = *solution
        .flow_rates
        .get(&edge_idx["Daughter2"])
        .unwrap_or(&0.0);
    ...
    println!(
        ...
        if p_inlet_pass { "PASS" } else { "FAIL" },
        p_inlet_err * 100.0
    );
    println!(
        "  [{}] P_bifurc vs Analytical: {:.4}% deviation",
        if p_bifurc_pass { "PASS" } else { "FAIL" },
        p_bifurc_err * 100.0
    );

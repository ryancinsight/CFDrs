...
mod tests {
    ...
    fn test_diffusion_solution() {
        ...
        assert!((u1 - 1.0).abs() < 1e-14, "Expected 1.0, got {} at (0.5,0.5,0,0)", u1);
        ...
        assert!((u_t1 - expected_decay).abs() < 1e-14, "Expected {}, got {} at t=1", expected_decay, u_t1);

        // Verify decay property: u(t=1) < u(t=0)
        let u_t0 = solution.exact_solution(0.5, 0.5, 0.0, 0.0);
        assert!(u_t1 < u_t0, "Solution should decay over time: u(t=1)={} should be < u(t=0)={}", u_t1, u_t0);
    }
    }

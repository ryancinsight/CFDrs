...
impl<T: CfdScalar + std::fmt::LowerExp> BifurcationFlow<T>
where
    T: FloatElement,
{
    ...
    pub fn new(width: T, length: T, angle: T) -> Self {
        ...
        let murray_factor = <T as FloatElement>::from_f64(0.79370052598);
        ...
        Self {
            ...
            geometry: Bifurcation2D::new_symmetric(
                ...
                length,
                daughter_width,
                length * <T as FloatElement>::from_f64(1.5),
                angle,
            ),
        }
    }
...
impl<T: CfdScalar + std::fmt::LowerExp> Benchmark<T> for BifurcationFlow<T>
where
    T: FloatElement,
{
    fn name(&self) -> &str {
    ...
    fn run(&self, config: &BenchmarkConfig<T>) -> Result<BenchmarkResult<T>> {
        ...
        let ny = (config.resolution as f64 * <T as NumericElement>::to_f64(ly)
            / <T as NumericElement>::to_f64(lx)) as usize;
            ...
            for j in 0..ny {
                ...
                if is_fluid {
                    fields.viscosity.set(
                        i,
                        j,
                        blood.apparent_viscosity(<T as FloatElement>::from_f64(100.0)),
                    );
                } else {
                    fields
                        .viscosity
                        .set(i, j, <T as FloatElement>::from_f64(0.0));
                }
            }
        }
        ...
        for _ in 0..config.max_iterations {
            ...
            let dt = config
                .time_step
                .unwrap_or_else(|| <T as FloatElement>::from_f64(0.01));
            let residual =
                solver.solve_time_step(&mut fields, dt, <T as FloatElement>::from_f64(0.0), rho)?;
        ...
        let rhs = <T as FloatElement>::from_f64(2.0) * d_daughter * d_daughter * d_daughter;
        let murray_deviation = <T as NumericElement>::to_f64(scalar::abs(lhs - rhs) / lhs);
        result.metrics.insert(
            "Murray Deviation".to_string(),
            <T as FloatElement>::from_f64(murray_deviation),
        );
    ...
    fn validate(&self, result: &BenchmarkResult<T>) -> Result<bool> {
        ...
        let last_residual = result
            ...
            .last()
            .copied()
            .unwrap_or_else(|| <T as FloatElement>::from_f64(1.0));
        Ok(last_residual < <T as FloatElement>::from_f64(1e-3))
    }
}

...
impl<
        T: RealField
            ...
            + std::fmt::LowerExp
            + std::fmt::Debug,
    > Benchmark<T> for SerpentineFlow<T>
{
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn run(&self, config: &BenchmarkConfig<T>) -> Result<BenchmarkResult<T>> {
        ...
        let props = FluidTrait::properties_at(
            &blood,
            T::from_f64(310.0).unwrap_or_else(num_traits::Zero::zero),
            T::from_f64(101325.0).unwrap_or_else(num_traits::Zero::zero),
        )?;
        ...
        let dean_number = re_target
            * Float::sqrt(
                self.geometry.width
                    / (T::from_f64(2.0).unwrap_or_else(num_traits::Zero::zero) * r_curv),
            );
    ...
    fn validate(&self, result: &BenchmarkResult<T>) -> Result<bool> {
        ...
        if last_residual > T::from_f64(5e-3).unwrap_or_else(num_traits::Zero::zero) {
            return Ok(false);
        }

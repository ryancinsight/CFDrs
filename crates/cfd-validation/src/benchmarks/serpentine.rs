...
impl<T: CfdScalar + std::fmt::LowerExp> SerpentineFlow<T>
where
    T: FloatElement,
{
...
impl<T: CfdScalar + std::fmt::LowerExp> Benchmark<T> for SerpentineFlow<T>
where
    T: FloatElement,
{
    fn name(&self) -> &str {
    ...
    fn run(&self, config: &BenchmarkConfig<T>) -> Result<BenchmarkResult<T>> {
        ...
        let pi: T = <T as FloatElement>::from_f64(std::f64::consts::PI);
        let r_curv = scalar::powf(
            self.geometry.period_length,
            <T as FloatElement>::from_f64(2.0),
        ) / (<T as FloatElement>::from_f64(4.0) * pi * pi * self.geometry.amplitude);
        let dean_number = re_target
            * scalar::sqrt(self.geometry.width / (<T as FloatElement>::from_f64(2.0) * r_curv));
    ...
    fn validate(&self, result: &BenchmarkResult<T>) -> Result<bool> {
        ...
        if last_residual > <T as FloatElement>::from_f64(5e-3) {
            return Ok(false);
        }

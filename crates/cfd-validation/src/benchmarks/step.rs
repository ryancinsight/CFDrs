...
impl<T: RealField + Copy + FloatElement> BackwardFacingStep<T> {
    ...
    fn calculate_reattachment_length(&self, _u_field: &DMatrix<T>) -> T {
        ...
        <T as FloatElement>::from_f64(6.0) * self.step_height
    }
}
...
impl<T: RealField + Copy + FloatElement> Benchmark<T> for BackwardFacingStep<T> {
    ...
    fn run(&self, config: &BenchmarkConfig<T>) -> Result<BenchmarkResult<T>> {
        ...
        for j in 0..ny / 2 {
            ...
            u[(j + ny / 2, 0)] = self.inlet_velocity
                * (<T as FloatElement>::from_f64(1.0)
                    - (y - <T as FloatElement>::from_f64(0.5))
                        * (y - <T as FloatElement>::from_f64(0.5)));
        }
        ...
        let reynolds = self.inlet_velocity * self.step_height / <T as FloatElement>::from_f64(0.01); // Assuming nu = 0.01
        ...
        for _iter in 0..config.max_iterations {
            let mut local_max_residual = <T as FloatElement>::from_f64(0.0);

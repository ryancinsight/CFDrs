...
impl<T: RealField> BackwardFacingStep<T> {
    ...
    fn calculate_reattachment_length(&self, u_field: &DMatrix<T>) -> T {
        ...
        T::from_f64(6.0).unwrap_or_else(|| T::from_i32(6).unwrap()) * self.step_height.clone()
    }
}
...
impl<T: RealField + FromPrimitive> Benchmark<T> for BackwardFacingStep<T> {
    ...
    fn run(&self, config: &BenchmarkConfig<T>) -> Result<BenchmarkResult<T>> {
        ...
        let h_inlet = self.channel_height.clone() - self.step_height.clone();
        for j in 0..ny/2 {
            let y = T::from_usize(j).unwrap_or_else(|| T::zero()) / 
                    T::from_usize(ny/2).unwrap_or_else(|| T::one());
            u[(j + ny/2, 0)] = self.inlet_velocity.clone() * 
                               (T::one() - (y.clone() - T::from_f64(0.5).unwrap()) * 

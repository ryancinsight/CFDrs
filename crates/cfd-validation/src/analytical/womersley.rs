...
impl<T: ValidationScalar> WomersleyFlow<T> {
    ...
    pub fn physiological_blood_flow() -> Self {
        Self {
            radius: scalar::from_f64(4.0e-3),                      // 4 mm
            density: scalar::from_f64(1060.0),                     // Blood density
            viscosity: scalar::from_f64(0.0035),                   // Pa·s (apparent)
            omega: scalar::from_f64(2.0 * PI * 1.2),               // ~72 bpm
            pressure_gradient_amplitude: scalar::from_f64(1000.0), // Pa/m
        }
    }
    ...
    pub fn womersley_number(&self) -> T {
        ...
        self.radius * scalar::sqrt(self.omega / nu)
    }
...
impl<T: ValidationScalar> AnalyticalSolution<T> for WomersleyFlow<T> {
    ...
    fn pressure(&self, x: T, _y: T, _z: T, t: T) -> T {
        ...
        let dpdx = self.pressure_gradient_amplitude;
        -dpdx * x * scalar::cos(self.omega * t)
    }
    ...
    fn domain_bounds(&self) -> [T; 6] {
        let length = scalar::from_f64::<T>(100.0) * self.radius; // 100R long pipe
        [
            scalar::zero::<T>(),
            length, // x: [0, L]
            -self.radius,
    ...
    fn velocity_scale(&self) -> T {
        scalar::abs(self.characteristic_velocity())
    }
}

...
mod time_integration_edge_tests {
    ...
    fn test_forward_euler_order() {
        let integrator = ForwardEuler;
        assert_eq!(
            <ForwardEuler as TimeIntegratorTrait<f64>>::order(&integrator),
            1,
            "Forward Euler is first-order accurate"
        );
        assert_eq!(
            <ForwardEuler as TimeIntegratorTrait<f64>>::workspace_size(&integrator),
            0,
            "Forward Euler requires no workspace"
        );
    }
    ...
    fn test_rk2_order() {
        let integrator = RungeKutta2;
        assert_eq!(
            <RungeKutta2 as TimeIntegratorTrait<f64>>::order(&integrator),
            2,
            "RK2 is second-order accurate"
        );
        assert!(
            <RungeKutta2 as TimeIntegratorTrait<f64>>::workspace_size(&integrator) > 0,
            "RK2 requires workspace"
        );
    }
    ...
    fn test_forward_euler_large_timestep() {
        ...
        assert!(
            y[0].is_finite(),
            "Solution remains stable with large timestep"
        );
    ...
    fn test_rk2_stiff_ode() {
        ...
        assert!(
            y[0].is_finite(),
            "Solution numerically stable for stiff ODE"
        );
    ...
    fn test_rk2_vs_euler_accuracy() {
        ...
        assert!(
            error_rk2 < error_euler,
            "RK2 more accurate than Forward Euler"
        );
    }
}

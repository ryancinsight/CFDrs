...
use super::integrators::{ForwardEuler, RungeKutta2, RungeKutta4, TimeIntegratorTrait, HALF, ONE_SIXTH, TWO};
...
impl TimeIntegrationValidator {
    ...
    pub fn validate_all<T: RealField + Copy + FromPrimitive + ToPrimitive>() -> Result<Vec<TimeIntegrationResult<T>>> {
    ...
    fn validate_exponential_decay<T: RealField + Copy + FromPrimitive + ToPrimitive>() -> Result<Vec<TimeIntegrationResult<T>>> {
        let mut results = Vec::new();
    ...
    fn validate_harmonic_oscillator<T: RealField + Copy + FromPrimitive + ToPrimitive>() -> Result<Vec<TimeIntegrationResult<T>>> {
        let mut results = Vec::new();
        ...
        for (name, integrator_fn) in [
            ("ForwardEuler", |y: &mut DVector<T>, t, dt| ForwardEuler.step(y, t, dt, &f)),
            ("RungeKutta2", |y: &mut DVector<T>, t, dt| RungeKutta2.step(y, t, dt, &f)),
            ("RungeKutta4", |y: &mut DVector<T>, t, dt| RungeKutta4.step(y, t, dt, &f)),
        ] {
            let mut y = y0.clone();
            let mut t = T::zero();

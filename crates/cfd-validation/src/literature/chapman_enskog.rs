...
impl<T: RealField + Copy + FromPrimitive> ChapmanEnskogValidation<T> {
...
impl<T: RealField + Copy + FromPrimitive + Copy> LiteratureValidation<T> for ChapmanEnskogValidation<T> {
    fn validate(&self) -> Result<ValidationReport<T>> {
        ...
        Ok(ValidationReport {
            test_name: format!("Chapman-Enskog {:?} at T={:.0}K", 
                self.gas_type,
            ...
            passed: true,
            details: format!("μ={:.2e} Pa·s, k={:.3e} W/(m·K)", 

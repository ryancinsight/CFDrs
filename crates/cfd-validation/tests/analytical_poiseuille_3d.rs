...
fn test_3d_poiseuille_stokes_flow() {
    ...
    for &i in &boundary_nodes {
        ...
        if x_proj <= min_boundary_proj + axial_tol {
            ...
            if u_mag < 0.0 {
                u_mag = 0.0;
            } // Clamp machine epsilon outside radius
            inlet_nodes.insert(i);
            let velocity = axis_dir * u_mag;
            boundary_conditions.insert(
                i,
                BoundaryCondition::VelocityInlet {
                    velocity: LetoVector3::new(velocity.x, velocity.y, velocity.z),
                },
            );
    ...
    // 4. Solve Stokes Flow
    let problem =
        StokesFlowProblem::new(mesh.clone(), fluid.clone(), boundary_conditions, n_corner);
    ...
    assert!(
        core_count > 0,
        "No core vertices found in evaluation region"
    );
    assert!(
        wall_count > 0,
        "No near-wall vertices found in evaluation region"
    );
    assert!(inlet_pressure_count > 0, "No inlet pressure samples found");
    assert!(
        outlet_pressure_count > 0,
        "No outlet pressure samples found"
    );

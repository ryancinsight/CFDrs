...
//! write_hdf5("field.h5", &datasets, &metadata).expect("expected value");
//! ```
...
mod tests {
    ...
    fn read_dataset_f64(file: &Hdf5File<MemCursor>, name: &str) -> Vec<f64> {
        ...
        buf.chunks_exact(8)
            .map(|c| f64::from_le_bytes(c.try_into().expect("expected value")))
            .collect()
    }
    ...
    fn shape_data_mismatch_is_rejected() {
        ...
        let err =
            write_hdf5(&path, &datasets, &[]).expect_err("shape/data mismatch must be rejected");

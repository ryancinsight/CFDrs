    ...
    pub fn validate_periodic(&self) -> Result<()> {
        for (name, bc) in &self.conditions {
            if let BoundaryCondition::Periodic { partner } = bc {
                ...
                let partner_bc = self.conditions.get(partner)
                    .ok_or_else(|| Error::InvalidConfiguration(
                        format!("Periodic boundary '{}' references non-existent partner '{}'", name, partner)
                    ))?;
                
                // Check partner is also periodic
                if let BoundaryCondition::Periodic { partner: partner_ref } = partner_bc {
                    // Check mutual reference
                    if partner_ref != name {
                        return Err(Error::InvalidConfiguration(
                            format!("Periodic boundaries '{}' and '{}' do not reference each other", name, partner)
                        ));
                    }
                } else {
                    return Err(Error::InvalidConfiguration(
                        format!("Periodic boundary '{}' partner '{}' is not periodic", name, partner)
                    ));
                }
            }
        }

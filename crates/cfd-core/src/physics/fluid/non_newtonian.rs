...
impl<T: RealField + FromPrimitive + Copy> CarreauYasuda<T> {
    ...
    pub fn blood() -> Self {
        // Values from literature (e.g., Gijsen et al.)
        Self::new(
            "Blood".to_string(),
            T::from_f64(1060.0).unwrap(),    // density
            T::from_f64(0.056).unwrap(),     // μ₀
            T::from_f64(0.0035).unwrap(),    // μ_inf
            T::from_f64(3.313).unwrap(),     // λ
            T::from_f64(0.3568).unwrap(),    // n
            T::from_f64(2.0).unwrap(),       // a (Carreau)
            T::from_f64(3600.0).unwrap(),    // cp
            T::from_f64(0.5).unwrap(),       // k
            T::from_f64(1540.0).unwrap(),    // speed of sound
        )
    }
}
...
impl<T: RealField + FromPrimitive + Copy> FluidTrait<T> for CarreauYasuda<T> {
    fn properties_at(&self, _temperature: T, _pressure: T) -> Result<FluidState<T>, Error> {
        // Return properties at low shear rate as default
        Ok(FluidState {
            density: self.density,
...
impl<T: RealField + FromPrimitive + Copy> Casson<T> {
    ...
    pub fn apparent_viscosity(&self, shear_rate: T) -> T {
        if shear_rate <= T::from_f64(1e-6).unwrap_or(T::zero()) {
             // Return high viscosity at near-zero shear
...
impl<T: RealField + FromPrimitive + Copy> FluidTrait<T> for Casson<T> {
    fn properties_at(&self, _temperature: T, _pressure: T) -> Result<FluidState<T>, Error> {
        Ok(FluidState {
            density: self.density,
            dynamic_viscosity: self.plastic_viscosity, // Approximation for interface
            specific_heat: self.specific_heat,
            thermal_conductivity: self.thermal_conductivity,
            speed_of_sound: self.speed_of_sound,
        })

...
mod tests {
    ...
    fn test_giersiepen_damage_increases_with_stress() {
        ...
        let d1 = model
            .damage_index(50.0, t)
            .expect("invariant: positive stress produces damage index");
        let d2 = model
            .damage_index(100.0, t)
            .expect("invariant: positive stress produces damage index");
        let d3 = model
            .damage_index(200.0, t)
            .expect("invariant: positive stress produces damage index");
    ...
    fn test_damage_increases_with_time() {
        ...
        let d1 = model
            .damage_index(tau, 0.1)
            .expect("invariant: positive exposure time produces damage index");
        let d2 = model
            .damage_index(tau, 1.0)
            .expect("invariant: positive exposure time produces damage index");
        let d3 = model
            .damage_index(tau, 10.0)
            .expect("invariant: positive exposure time produces damage index");
    ...
    fn test_heuser_opitz_threshold() {
        ...
        let d_below = model
            .damage_index(100.0, 1.0)
            .expect("invariant: below-threshold stress is valid");
        let d_above = model
            .damage_index(200.0, 1.0)
            .expect("invariant: above-threshold stress is valid");
    ...
    fn test_nih_calculation() {
        let calc: HemolysisCalculator<f64> =
            HemolysisCalculator::new(HemolysisModel::default(), 0.45, 15.0, 5e-3, 1e-4)
                .expect("invariant: valid hemolysis parameters are accepted");
    ...
    fn test_critical_shear_stress() {
        ...
        let tau_crit = calc
            ...
            .expect("invariant: positive exposure produces critical shear stress");

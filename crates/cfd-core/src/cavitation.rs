...
impl<T: RealField + FromPrimitive + Copy> RayleighPlesset<T> {
    ...
    pub fn bubble_pressure(&self, _ambient_pressure: T) -> T {
        // p_B = p_v - 2σ/R - 4μṘ/R
        let surface_term =
            T::from_f64(2.0).unwrap_or_else(|| T::one()) * self.surface_tension / self.radius;
        let viscous_term =
            T::from_f64(4.0).unwrap_or_else(|| T::one()) * self.viscosity * self.velocity
                / self.radius;
    ...
    pub fn acceleration(&self, ambient_pressure: T) -> T {
        ...
        let kinetic_term =
            T::from_f64(1.5).unwrap_or_else(|| T::one()) * self.velocity * self.velocity;
    ...
    pub fn rayleigh_collapse_time(&self, pressure_difference: T) -> T {
        ...
        T::from_f64(0.915).unwrap_or_else(|| T::one())
            * self.initial_radius
    ...
    pub fn mass_transfer_rate(
        ...
        match self {
            ...
            CavitationModel::SchnerrSauer {
                ...
                let denominator = T::from_f64(4.0 * std::f64::consts::PI)
                    .unwrap_or_else(|| T::one())
                    * n_b
                    * (T::one() - alpha);
                if denominator > T::from_f64(1e-10).unwrap_or_else(|| T::one()) {
                    let radius_cubed =
                        T::from_f64(3.0).unwrap_or_else(|| T::one()) * alpha / denominator;
                    let radius =
                        radius_cubed.powf(T::from_f64(1.0 / 3.0).unwrap_or_else(|| T::one()));
                    // Mass transfer rate
                    let pressure_diff = pressure - vapor_pressure;
                    ...
                    sign * T::from_f64(3.0).unwrap_or_else(|| T::one())
                        * alpha
                        * (T::one() - alpha)
                        * (T::from_f64(2.0 / 3.0).unwrap_or_else(|| T::one()) * pressure_diff.abs()
                            / density_liquid)
                            .sqrt()
                        / radius
                    T::zero()
...
impl<T: RealField + FromPrimitive + Copy> VenturiCavitation<T> {
    ...
    pub fn throat_pressure(&self, inlet_velocity: T) -> T {
        ...
        let dynamic_diff = T::from_f64(0.5).unwrap_or_else(|| T::one())
            * self.fluid_density
    ...
    pub fn inception_velocity(&self) -> T {
        ...
        let sigma_inception =
            T::from_f64(constants::CAVITATION_INCEPTION_THRESHOLD).unwrap_or_else(|| T::one());
        // σ = (p_inlet - p_v - 0.5*ρ*(v_throat² - v_inlet²)) / (0.5*ρ*v_throat²)
        // Solving for v_inlet when σ = σ_inception
        let ar2 = area_ratio * area_ratio;
        let numerator = T::from_f64(2.0).unwrap_or_else(|| T::one())
            * (self.inlet_pressure - self.vapor_pressure);
...
impl<T: RealField + FromPrimitive + Copy> CavitationDamage<T> {
    ...
    pub fn erosion_rate(&self) -> T {
        ...
        if self.collapse_pressure > threshold {
            let intensity = (self.collapse_pressure - threshold)
                .powf(T::from_f64(2.0).unwrap_or_else(|| T::one()));
            T::from_f64(1e-12).unwrap_or_else(|| T::one())
                * intensity
                * self.collapse_frequency
                * self.affected_area

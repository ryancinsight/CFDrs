...
impl FluidDynamicsService {
    ...
    pub fn pipe_pressure_drop<T: RealField + FromPrimitive + Copy>(
        ...
        let reynolds = Self::reynolds_number(fluid, velocity.clone(), diameter.clone());
        let friction_factor = Self::friction_factor(reynolds, diameter.clone(), roughness)?;

        let two = T::one() + T::one();

        Ok(friction_factor * length * fluid.density.clone() * velocity.clone() * velocity / (two * diameter))
    }
    ...
    fn colebrook_white_friction_factor<T: RealField + FromPrimitive + Copy>(
        ...
        for _ in 0..max_iterations {
            ...
            if (f_new.clone() - f.clone()).abs() < tolerance {
                return Ok(f_new);
            }
            f = f_new;
        }

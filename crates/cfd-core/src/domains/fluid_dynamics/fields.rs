...
impl<T: RealField + Copy> PressureField<T> {
    ...
    /// Create a mutable view of the pressure field
...
impl<T: RealField + Copy> ScalarField<T> {
    /// Get scalar value at a specific grid point
    pub fn get(&self, i: usize, j: usize, k: usize) -> Option<T> {
        let (nx, ny, _) = self.dimensions;
        let idx = k * nx * ny + j * nx + i;
        self.values.get(idx).copied()
    ...
    /// Set scalar value at a specific grid point
    pub fn set(&mut self, i: usize, j: usize, k: usize, value: T) {
        let (nx, ny, _) = self.dimensions;
        let idx = k * nx * ny + j * nx + i;
        if let Some(s) = self.values.get_mut(idx) {
            *s = value;
        }
    }
    
    /// Create a view of the scalar field as a slice
    pub fn as_slice(&self) -> &[T] {
        &self.values
    }

...
mod tests {
    ...
    fn safe_conversion_uses_eunomia_float_construction() {
        ...
        assert_eq!(<f64 as SafeFromF64>::try_from_f64(-3.75).expect("expected value"), -3.75);
        assert_eq!(<f64 as SafeFromI32>::from_i32_or_zero(-7), -7.0);
        assert_eq!(<f64 as SafeFromI32>::try_from_i32(11).expect("expected value"), 11.0);
        assert_eq!(<f64 as SafeFromUsize>::from_usize_or_one(13), 13.0);
        assert_eq!(<f64 as SafeFromUsize>::try_from_usize(17).expect("expected value"), 17.0);
    }
}

...
impl Laplacian2DKernel {
    ...
    /// use cfd_core::gpu::kernels::laplacian::{Laplacian2DKernel, BoundaryType};
    ///
    ...
    fn execute_cpu(
        ...
        let dx_inv2_64 = 1.0f64 / ((dx as f64) * (dx as f64));
        let dy_inv2_64 = 1.0f64 / ((dy as f64) * (dy as f64));
        ...
        for y in 0..ny {
            for x in 0..nx {
                ...
                if x > 0 && x < nx - 1 {
                    ...
                    laplacian +=
                        ((left as f64) - 2.0f64 * (center as f64) + (right as f64)) * dx_inv2_64;
                } else if x == 0 {
                    match bc {
                        BoundaryType::Dirichlet => {
                            ...
                            laplacian += (-2.0f64 * (center as f64)) * dx_inv2_64;
                        }
                        BoundaryType::Neumann => {
                            ...
                            if nx >= 4 {
                                ...
                                let u2 = field[y * nx + 2];
                                let u3 = field[y * nx + 3];
                                let d2x = (2.0 * u0 - 5.0 * u1 + 4.0 * u2 - u3) as f64 * dx_inv2_64;
                                laplacian += d2x;
                            } else {
                                let right = field[y * nx + (x + 1)];
                                laplacian += ((right as f64) - 2.0f64 * (center as f64)
                                    + (right as f64))
                                    * dx_inv2_64;
                            }
                        }
                        BoundaryType::Periodic => {
                            ...
                            let center = field[idx];
                            let right = field[y * nx + (x + 1)];
                            laplacian += ((left as f64) - 2.0f64 * (center as f64)
                                + (right as f64))
                                * dx_inv2_64;
                        }
                    }
                ...
                if y > 0 && y < ny - 1 {
                    ...
                    laplacian +=
                        ((bottom as f64) - 2.0f64 * (center as f64) + (top as f64)) * dy_inv2_64;
                } else if y == 0 {
                    match bc {
                        BoundaryType::Dirichlet => {
                            ...
                            laplacian += (-2.0f64 * (center as f64)) * dy_inv2_64;
                        }
                        BoundaryType::Neumann => {
                            ...
                            if ny >= 4 {
                                ...
                                let u2 = field[2 * nx + x];
                                let u3 = field[3 * nx + x];
                                let d2y = (2.0 * u0 - 5.0 * u1 + 4.0 * u2 - u3) as f64 * dy_inv2_64;
                                laplacian += d2y;
                            } else {
                                let top = field[(y + 1) * nx + x];
                                laplacian += ((top as f64) - 2.0f64 * (center as f64)
                                    + (top as f64))
                                    * dy_inv2_64;
                            }
                        }
                        BoundaryType::Periodic => {
                            ...
                            let center = field[idx];
                            let top = field[(y + 1) * nx + x];
                            laplacian += ((bottom as f64) - 2.0f64 * (center as f64)
                                + (top as f64))
                                * dy_inv2_64;
                        }
                    }
    ...
    fn execute_gpu(
        ...
        encoder.copy_buffer_to_buffer(
            ...
            &staging_buffer,
            0,
            (field.len() * std::mem::size_of::<f32>()) as u64,
        );

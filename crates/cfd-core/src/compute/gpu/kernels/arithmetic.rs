...
impl FieldAddKernel {
    ...
    fn execute_gpu(&self, a: &[f32], b: &[f32], result: &mut [f32]) -> Result<(), &'static str> {
        ...
        let data_size = std::mem::size_of_val(a) as u64;
        let aligned_size = data_size.div_ceil(256) * 256; // Align to 256 bytes
...

        {
            ...
            let workgroups = a.len().div_ceil(64) as u32;
...
impl FieldMulKernel {
    ...
    fn execute_gpu(
        ...
        let data_size = std::mem::size_of_val(field) as u64;
        let aligned_size = data_size.div_ceil(256) * 256; // Align to 256 bytes
...

        {
            ...
            let workgroups = field.len().div_ceil(64) as u32;
            compute_pass.dispatch_workgroups(workgroups, 1, 1);
        }

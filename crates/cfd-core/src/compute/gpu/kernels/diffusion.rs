//! GPU diffusion kernel implementation
...
pub struct GpuDiffusionKernel<T: RealField + Copy> {
...
impl<T: RealField + Copy> ComputeKernel<T> for GpuDiffusionKernel<T> {
    ...
    fn complexity(&self, size: usize) -> usize {
        // Approximately 15 FLOPs per grid point for 3D Laplacian
        size * 15
    }
...
impl<T: RealField + Copy> GpuKernel<T> for GpuDiffusionKernel<T> {
...
impl<T: RealField + Copy> std::fmt::Debug for GpuDiffusionKernel<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("GpuDiffusionKernel").finish()
    }
}

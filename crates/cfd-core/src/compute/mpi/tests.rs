...
mod tests {
    ...
    fn test_global_extents_creation() {
        ...
        let extents_3d = GlobalExtents::new_3d(
            50, 40, 30,
            (0.0, 2.0, 0.0, 1.5, 0.0, 3.0)
        );
    ...
    fn test_domain_decomposition_1d() {
        ...
        let subdomain = super::decomposition::DomainDecomposition::decompose_1d(
    ...
    fn test_domain_decomposition_2d() {
        ...
        let subdomain = super::decomposition::DomainDecomposition::decompose_2d(
    ...
    fn test_ghost_cell_manager_creation() {
        ...
        let _manager_type: std::marker::PhantomData<
            super::ghost_cells::GhostCellManager<f64>
    ...
    fn test_neighbor_direction_enum() {
        ...
        let _left = super::decomposition::NeighborDirection::Left;
        let _right = super::decomposition::NeighborDirection::Right;
        let _bottom = super::decomposition::NeighborDirection::Bottom;
        let _top = super::decomposition::NeighborDirection::Top;
        let _front = super::decomposition::NeighborDirection::Front;
        let _back = super::decomposition::NeighborDirection::Back;
    }

    #[test]
    fn test_neighbor_info_creation() {
        let neighbor_info = super::decomposition::NeighborInfo {
            direction: super::decomposition::NeighborDirection::Left,
            overlap: 1,
        };

        assert_eq!(neighbor_info.overlap, 1);
        match neighbor_info.direction {
            super::decomposition::NeighborDirection::Left => {},
    ...
    fn test_ghost_cell_update_config() {
        let config = super::ghost_cells::GhostCellUpdate {
            max_iterations: 10,
            tolerance: 1e-6,
            blocking: true,
        };
    ...
    fn test_ghost_cell_stats_default() {
        let stats = super::ghost_cells::GhostCellStats::default();
    ...
    fn test_mpi_error_types() {
        ...
        let init_error = super::error::MpiError::InitializationError("test".to_string());
        let decomp_error = super::error::MpiError::DecompositionError("test".to_string());
        let comm_error = super::error::MpiError::CommunicationError("test".to_string());
        let not_avail = super::error::MpiError::NotAvailable("test".to_string());

        // Check error messages
        match init_error {
            super::error::MpiError::InitializationError(msg) => assert_eq!(msg, "test"),
            _ => panic!("Wrong error type"),
        }

        match decomp_error {
            super::error::MpiError::DecompositionError(msg) => assert_eq!(msg, "test"),
            _ => panic!("Wrong error type"),
        }

        match comm_error {
            super::error::MpiError::CommunicationError(msg) => assert_eq!(msg, "test"),
            _ => panic!("Wrong error type"),
        }

        match not_avail {
            super::error::MpiError::NotAvailable(msg) => assert_eq!(msg, "test"),
            _ => panic!("Wrong error type"),
        }
    }
    ...
    fn test_domain_decomposition_neighbors_1d() {
        ...
        let neighbors = super::decomposition::DomainDecomposition::compute_neighbors(
            &subdomain, &global, 4
        ).unwrap();
        ...
        if let Some(left_info) = neighbors.get(&0) {
            assert_eq!(left_info.direction, super::decomposition::NeighborDirection::Left);
            assert_eq!(left_info.overlap, 1);
        }

        if let Some(right_info) = neighbors.get(&2) {
            assert_eq!(right_info.direction, super::decomposition::NeighborDirection::Right);
    ...
    fn test_find_rank_for_global_index() {
        ...
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(10, &global, 4).unwrap(),
            0
        );
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(30, &global, 4).unwrap(),
            1
        );
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(60, &global, 4).unwrap(),
            2
        );
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(85, &global, 4).unwrap(),
            3
        );

        // Test boundary cases
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(24, &global, 4).unwrap(),
            0
        );
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(25, &global, 4).unwrap(),
            1
        );
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(49, &global, 4).unwrap(),
            1
        );
        assert_eq!(
            super::decomposition::DomainDecomposition::find_rank_for_global_i(50, &global, 4).unwrap(),
            2
        );
    }
    ...
    fn test_distributed_linear_operator_trait() {
        ...
        use super::distributed_solvers::DistributedLinearOperator;
    ...
    fn test_parallel_preconditioner_types() {
        ...
        use super::distributed_solvers::{BlockJacobiPreconditioner, AdditiveSchwarzPreconditioner};

        let _jacobi_type: std::marker::PhantomData<BlockJacobiPreconditioner<f64, super::distributed_solvers::DistributedLaplacian2D<f64>>> =
            std::marker::PhantomData;

        let _schwarz_type: std::marker::PhantomData<AdditiveSchwarzPreconditioner<f64, super::distributed_solvers::DistributedLaplacian2D<f64>>> =
            std::marker::PhantomData;
    }
    ...
    fn test_distributed_gmres_solver_type() {
        ...
        use super::distributed_solvers::DistributedGMRES;

        let _gmres_type: std::marker::PhantomData<
            DistributedGMRES<f64, super::distributed_solvers::DistributedLaplacian2D<f64>, BlockJacobiPreconditioner<f64, super::distributed_solvers::DistributedLaplacian2D<f64>>>
        > = std::marker::PhantomData;
    }
    ...
    fn test_parallel_io_types() {
        ...
        use super::distributed_solvers::parallel_io::{ParallelVtkWriter, ParallelHdf5Writer};

        let _vtk_writer: std::marker::PhantomData<ParallelVtkWriter<f64>> = std::marker::PhantomData;
        let _hdf5_writer: std::marker::PhantomData<ParallelHdf5Writer<f64>> = std::marker::PhantomData;
    }

    #[test]
    fn test_load_balance_metrics_creation() {
        let metrics = super::decomposition::LoadBalanceMetrics {
            max_load: 1000,
            min_load: 800,
            avg_load: 900.0,
    ...
    fn test_refinement_criteria_creation() {
        let criteria = super::decomposition::RefinementCriteria {
    ...
    fn test_adaptive_mesh_refinement_creation() {
        ...
        let amr = super::decomposition::AdaptiveMeshRefinement::new(5, criteria, None);
    ...
    fn test_adaptive_mesh_refinement_needs_coarsening() {
        ...
        let mut amr = super::decomposition::AdaptiveMeshRefinement::new(5, criteria, None);
    ...
    fn test_adaptive_mesh_refinement_level_management() {
        ...
        let mut amr = super::decomposition::AdaptiveMeshRefinement::new(3, criteria, None);
    ...
    fn test_load_balance_metrics_calculation() {
        ...
        // Test with imbalance
        let imbalanced_metrics = super::decomposition::LoadBalanceMetrics {
            max_load: 1500,
            min_load: 500,
            avg_load: 1000.0,
            imbalance_ratio: 1.5,

...
fn test_backend_selection_feature_gate() {
    ...
    #[cfg(feature = "gpu")]
    assert_eq!(backend, Backend::Gpu, "GPU feature enabled: should select GPU backend");
    
    #[cfg(not(feature = "gpu"))]
    assert_eq!(backend, Backend::Cpu, "GPU feature disabled: should select CPU backend");
}
...
fn test_compute_squares_integers_correctness() {
    ...
    assert_eq!(result.len(), expected.len(), "Output length must match input");
...
fn test_extreme_values() {
    ...
    for (i, (&result, &expected_val)) in small_result.iter().zip(small_expected.iter()).enumerate() {
        ...
        assert!(
            ...
            i, result, expected_val, rel_error
        );
    }
...
fn test_mixed_signs() {
    ...
    assert_eq!(result, expected, "Squares of negative numbers must be positive");
}
...
fn test_linear_scaling_property() {
    ...
    for &size in &sizes {
        ...
        assert_eq!(result.len(), size, "Output size must match input for size {}", size);
...
fn test_cfd_application_scenario() {
    ...
    for (i, &val) in u_squared.iter().enumerate() {
        ...
        assert_eq!(val, velocity_u[i] * velocity_u[i], "Must match analytical result");
    }
}

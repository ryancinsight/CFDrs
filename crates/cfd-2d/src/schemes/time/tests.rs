...
mod unit {
    ...
    fn test_bdf2_convergence_order() {
        ...
        assert!(
            ratio1 > 2.5,
            "Convergence ratio 1: {ratio1} should be > 2.5"
        );
        assert!(
            ratio2 > 2.5,
            "Convergence ratio 2: {ratio2} should be > 2.5"
        );
    ...
    fn test_bdf3_convergence_order() {
        ...
        for &dt in &dt_values {
            ...
            for step in 2..n_steps {
                let t = (step as f64) * dt;
                let y_next =
                    integrator.step_with_history(f, &y_curr, Some(&y_prev), Some(&y_prev2), t, dt);
                y_prev2 = y_prev;
                y_prev = y_curr;
                y_curr = y_next;
            }
        ...
        assert!(
            ratio1 > 4.0,
            "Convergence ratio 1: {ratio1} should be > 4.0 for BDF3"
        );
        assert!(
            ratio2 > 4.0,
            "Convergence ratio 2: {ratio2} should be > 4.0 for BDF3"
        );
    ...
    fn test_backward_euler_order_accuracy() {
        ...
        assert_eq!(
            TimeIntegrator::<f64>::new(TimeScheme::BackwardEuler).order(),
            1
        );
    }
    ...
    fn test_backward_euler_convergence_order() {
        ...
        assert!(
            ratio1 > 1.5,
            "Convergence ratio 1: {ratio1} should be > 1.5"
        );
        assert!(
            ratio2 > 1.5,
            "Convergence ratio 2: {ratio2} should be > 1.5"
        );
    ...
    fn test_crank_nicolson_order_accuracy() {
        ...
        assert_eq!(
            TimeIntegrator::<f64>::new(TimeScheme::CrankNicolson).order(),
            2
        );
    }
    ...
    fn test_adams_bashforth2_order_accuracy() {
        ...
        assert_eq!(
            TimeIntegrator::<f64>::new(TimeScheme::AdamsBashforth2).order(),
            2
        );
    }
    ...
    fn test_adams_bashforth2_convergence_order() {
        ...
        assert!(
            ratio1 > 2.0,
            "Convergence ratio 1: {ratio1} should be > 2.0 for AB2"
        );
        assert!(
            ratio2 > 2.0,
            "Convergence ratio 2: {ratio2} should be > 2.0 for AB2"
        );

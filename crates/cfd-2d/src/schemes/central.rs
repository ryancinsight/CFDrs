...
impl<T: RealField + FromPrimitive> SpatialDiscretization<T> for CentralDifference<T> {
...
impl<T: RealField + FromPrimitive> SpatialDiscretization<T> for FourthOrderCentral<T> {
    fn compute_derivative(&self, grid: &Grid2D<T>, i: usize, j: usize) -> T {

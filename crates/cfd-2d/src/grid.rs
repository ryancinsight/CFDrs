...
impl<T: RealField + Copy + FromPrimitive + Copy> StructuredGrid2D<T> {
    ...
    pub fn new(nx: usize, ny: usize, x_min: T, x_max: T, y_min: T, y_max: T) -> Result<Self> {
        ...
        let dx = (x_max - x_min) / cfd_core::numeric::from_usize(nx)?;
        let dy = (y_max - y_min) / cfd_core::numeric::from_usize(ny)?;
        Ok(Self {
            nx,
            ny,
...
impl<T: RealField + Copy + FromPrimitive + Copy> Grid2D<T> for StructuredGrid2D<T> {
    ...
    fn cell_center(&self, i: usize, j: usize) -> Result<Vector2<T>> {
        ...
        let x = self.bounds.0 + (cfd_core::numeric::from_usize(i)? + half) * self.dx;
        let y = self.bounds.2 + (cfd_core::numeric::from_usize(j)? + half) * self.dy;
        Ok(Vector2::new(x, y))
    }

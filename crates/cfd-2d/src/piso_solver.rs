...
impl<T: RealField + FromPrimitive + Copy> PisoSolver<T> {
    ...
    fn compute_momentum_coefficients(
        ...
        for i in 1..self.grid.nx - 1 {
            for j in 1..self.grid.ny - 1 {
                ...
                let u_e = (state.velocity[(i, j)].x + state.velocity[(i + 1, j)].x) / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                let u_w = (state.velocity[(i - 1, j)].x + state.velocity[(i, j)].x) / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                let v_n = (state.velocity[(i, j)].y + state.velocity[(i, j + 1)].y) / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                let v_s = (state.velocity[(i, j - 1)].y + state.velocity[(i, j)].y) / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                ...
                match self.config.convection_scheme {
                    ...
                    ConvectionScheme::Hybrid => {
                        ...
                        if pe_x.abs() < T::from_f64(2.0).unwrap_or_else(|| T::zero()) {
                            // Central difference
                            workspace.a_e[(i, j)] = diff_x - conv_e / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                            workspace.a_w[(i, j)] = diff_x + conv_w / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                        } else {
                            // Upwind
                            workspace.a_e[(i, j)] = diff_x + T::max(T::zero(), -conv_e);
                            workspace.a_w[(i, j)] = diff_x + T::max(T::zero(), conv_w);
                        ...
                        if pe_y.abs() < T::from_f64(2.0).unwrap_or_else(|| T::zero()) {
                            workspace.a_n[(i, j)] = diff_y - conv_n / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                            workspace.a_s[(i, j)] = diff_y + conv_s / T::from_f64(2.0).unwrap_or_else(|| T::zero());
                        } else {
                            workspace.a_n[(i, j)] = diff_y + T::max(T::zero(), -conv_n);
                            workspace.a_s[(i, j)] = diff_y + T::max(T::zero(), conv_s);
                        }
    ...
    fn gauss_seidel_momentum(
        ...
        for i in 1..self.grid.nx - 1 {
            for j in 1..self.grid.ny - 1 {
                ...
                if workspace.a_p[(i, j)].abs() <= T::from_f64(1e-14).unwrap_or_else(|| T::zero()) {
                    continue;
                }
    ...
    fn solve_pressure_correction(
        ...
        let dx2 = dx * dx;
        let dy2 = dy * dy;
        let factor = T::from_f64(2.0).unwrap_or_else(|| T::zero()) * (dx2 + dy2);

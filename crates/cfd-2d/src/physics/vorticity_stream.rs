...
impl<T: RealField + Copy + FromPrimitive + Copy> Default for VorticityStreamConfig<T> {
    fn default() -> Self {
        let base = cfd_core::solver::SolverConfig::builder()
            ...
            .tolerance(cfd_core::numeric::from_f64(DEFAULT_TOLERANCE)?)
            .build();
        Self {
            base,
            time_step: cfd_core::numeric::from_f64(0.001)?, // Default time step
            stream_tolerance: cfd_core::numeric::from_f64(DEFAULT_TOLERANCE)?,
            vorticity_tolerance: cfd_core::numeric::from_f64(DEFAULT_TOLERANCE)?,
            sor_omega: cfd_core::numeric::from_f64(SOR_OPTIMAL_FACTOR)?,
        }
    }
...
impl<T: RealField + Copy + FromPrimitive + Send + Sync> VorticityStreamSolver<T> {
    ...
    fn solve_stream_function(&mut self) -> Result<()> {
        ...
        let denominator = cfd_core::numeric::from_f64(GRADIENT_FACTOR)?
            * (T::one() / dx2 + T::one() / dy2);
        ...
        for i in 1..self.nx - 1 {
            for j in 1..self.ny - 1 {
                ...
                let d2wdx2 = (omega_old[i + 1][j]
                    - cfd_core::numeric::from_f64(GRADIENT_FACTOR)? * omega_old[i][j]
                    + omega_old[i - 1][j])
                    / dx2;
    ...
    fn update_velocity(&mut self) {
        let two_dx = cfd_core::numeric::from_f64(GRADIENT_FACTOR)? * self.dx;
        let two_dy = cfd_core::numeric::from_f64(GRADIENT_FACTOR)? * self.dy;
                // u = ∂ψ/∂y

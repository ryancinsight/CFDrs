...
impl<T: RealField + Copy> Default for VremanConfig<T> {
    fn default() -> Self {
        Self {
            c_v: T::from_f64(0.07), // Standard value from literature
            filter_width: T::ONE,
        }
    }
}
...
impl<T: RealField + Copy> VremanModel<T> {
    ...
    fn compute_vreman_viscosity(&self, velocity_gradient: &Array2<T>) -> T {
        ...
        if alpha_squared <= T::from_f64(1e-12) {
            return T::ZERO;
        }
        ...
        let b12 = beta[[0, 1]];
        let b22 = beta[[1, 1]];
        let b33 = (b11 + b22) * T::from_f64(0.5); // Isotropic assumption
        ...
        let sqrt_ratio = NumericElement::sqrt(ratio);
...
impl<T: RealField + Copy> Default for VremanModel<T> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_vreman_numerical_stability() {
        ...
        let mut small_gradient = zero_gradient();

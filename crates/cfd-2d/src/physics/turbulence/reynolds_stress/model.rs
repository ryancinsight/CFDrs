...
pub struct ReynoldsStressModel<T: RealField + Copy + FromPrimitive + ToPrimitive> {
    ...
    pub(super) c4: T,
    pub(super) c5: T,
    /// Active pressure-strain closure model.
    pub pressure_strain_model: PressureStrainModel,
    pub(super) _wall_reflection: bool,
...
impl<T: RealField + Copy + FromPrimitive + ToPrimitive> ReynoldsStressModel<T> {
    ...
    pub fn new(nx: usize, ny: usize) -> Self {
        ...
        Self {
            ...
            _wall_reflection: true,
            _curvature_correction: true,
        }
    }
    ...
    pub fn set_wall_distance(&mut self, wall_distance: Vec<T>) {
        assert_eq!(
            wall_distance.len(),
            self.nx * self.ny,
            "Wall distance vector must have {} elements",
            self.nx * self.ny
        );

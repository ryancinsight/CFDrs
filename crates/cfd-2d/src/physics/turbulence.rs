...
impl<T: RealField + Copy + FromPrimitive + Copy> KEpsilonModel<T> {
        ...
        for i in 0..self.nx {
            for j in 0..self.ny {
                ...
                let eps_val = self.epsilon[i][j]
                    .max(T::from_f64(constants::EPSILON_MIN).unwrap_or_else(|| T::zero()));
        ...
        for &(i_wall, j_wall) in wall_boundaries {
            ...
            if y_plus < T::from_f64(5.0).unwrap_or_else(|| T::zero()) {
                ...
                self.k[i_near][j_near] =
                    u_tau * u_tau * y_plus / T::from_f64(11.0).unwrap_or_else(|| T::zero());
            } else {
            ...
            if y_plus < T::from_f64(5.0).unwrap_or_else(|| T::zero()) {
                // Viscous sublayer
                let omega_visc =
                    T::from_f64(60.0).unwrap_or_else(|| T::zero()) * nu / (beta_1 * y * y);
    ...
    pub fn solve_transport_equations(
        ...
        for i in 1..self.nx - 1 {
            for j in 1..self.ny - 1 {
                ...
                let production = T::from_f64(2.0).unwrap_or_else(|| T::zero())
                    * self.nu_t[i][j]
                    * (s11 * s11
                        + T::from_f64(2.0).unwrap_or_else(|| T::zero()) * s12 * s12
                        + s22 * s22);
    ...
    fn calculate_diffusion(
        ...
        let d2f_dx2 = (field[i + 1][j]
            - T::from_f64(2.0).unwrap_or_else(|| T::zero()) * field[i][j]
            + field[i - 1][j])
            / (dx * dx);

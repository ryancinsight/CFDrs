...
mod tvd_limiter_edge_cases {
    ...
    fn test_local_maximum() {
        let limiters: Vec<Box<dyn TvdLimiter<f64>>> =
            vec![Box::new(Superbee), Box::new(VanLeer), Box::new(Minmod)];
    ...
    fn test_monotonic_increase() {
        ...
        assert!(
            (face - 2.5).abs() < 1e-10,
            "Monotonic sequence should use high-order"
        );
    }
    ...
    fn test_upwind_always_first_order() {
        ...
        assert_eq!(
            face, 2.0,
            "Upwind interpolation should return central value"
        );
    }
    ...
    fn test_limiter_names() {
        ...
        assert_eq!(
            <MonotonizedCentral as TvdLimiter<f64>>::name(&MonotonizedCentral),
            "MonotonizedCentral"
        );
        assert_eq!(
            <UpwindLimiter as TvdLimiter<f64>>::name(&UpwindLimiter),
            "Upwind"
        );
    }
}

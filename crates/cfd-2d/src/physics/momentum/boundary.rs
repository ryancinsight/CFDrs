    ...
    for (name, bc) in boundaries {
        if let BoundaryCondition::Periodic { partner } = bc {
            ...
            if !boundaries.contains_key(partner) {
                return Err(BoundaryError::InvalidRegion(format!(
                    "Periodic partner '{}' not found for boundary '{}'",
                    partner, name
                )));
            }
            ...
            if let Some(partner_bc) = boundaries.get(partner) {
                if let BoundaryCondition::Periodic { partner: p2 } = partner_bc {
                    if p2 != name {
                        return Err(BoundaryError::InvalidRegion(format!(
                            "Periodic mismatch: '{}' points to '{}', but '{}' points to '{}'",
                            name, partner, partner, p2
                        )));
                    }
                } else {
                    return Err(BoundaryError::InvalidRegion(format!(
                        "Periodic partner '{}' is not periodic",
                        partner
                    )));
                }
            }
    ...
    for (b1_name, b2_name) in corners {
        ...
        for component in [MomentumComponent::U, MomentumComponent::V] {
            if let (Some(v1), Some(v2)) =
                (get_dirichlet_value(b1, component), get_dirichlet_value(b2, component))
            {
                let diff = (v1 - v2).abs();
                ...
                if diff > epsilon {
                    ...
                    return Err(BoundaryError::InvalidRegion(format!(
                        "Corner conflict at {}-{}: Component {:?} values mismatch ({:?} vs {:?})",
                        b1_name, b2_name, component, v1, v2
                    )));
                }
            }
        }

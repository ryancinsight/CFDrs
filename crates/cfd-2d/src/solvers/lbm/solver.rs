...
pub struct LbmSolver<T: RealField + Copy> {
    ...
    f: Vec<Vec<[T; 9]>>,
    /// Distribution functions for streaming
    f_stream: Vec<Vec<[T; 9]>>,
    ...
    T: Send + Sync + std::fmt::LowerExp,
{
    ...
    pub fn new(config: LbmConfig<T>, grid: &StructuredGrid2D<T>) -> Self {
        ...
        let f_stream = vec![vec![[T::zero(); 9]; nx]; ny];
    ...
    pub fn step(
        ...
        // Streaming step
        StreamingOperator::stream(&self.f, &mut self.f_stream);
        std::mem::swap(&mut self.f, &mut self.f_stream);

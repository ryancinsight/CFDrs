...
pub fn compute_momentum<T: RealField + Copy + FromPrimitive>(
    f_node: &[T; 9],
) -> [T; 2] {
    let mut px = T::zero();
    let mut py = T::zero();
...
pub fn compute_kinetic_energy<T: RealField + Copy>(
    density: T,
    velocity: &[T; 2],
) -> T {
...
pub fn compute_vorticity<T: RealField + Copy>(
    ...
    // Central differences
    let dv_dx = (velocity[j][i + 1][1] - velocity[j][i - 1][1]) / (T::from_f64(2.0).unwrap_or_else(T::zero) * dx);
    let du_dy = (velocity[j + 1][i][0] - velocity[j - 1][i][0]) / (T::from_f64(2.0).unwrap_or_else(T::zero) * dy);

...
impl<T: RealField + Copy> EnergyEquationSolver<T> {
    ...
    pub fn solve_explicit(
        ...
        for i in 1..self.nx-1 {
            for j in 1..self.ny-1 {
                ...
                let d2t_dx2 = (self.temperature[i+1][j] - T::from_f64(2.0).unwrap_or_else(|| T::zero()) * t 
                    + self.temperature[i-1][j]) / (dx * dx);
                let d2t_dy2 = (self.temperature[i][j+1] - T::from_f64(2.0).unwrap_or_else(|| T::zero()) * t 
                    + self.temperature[i][j-1]) / (dy * dy);
    ...
    pub fn nusselt_number(&self, wall_temp: T, bulk_temp: T, characteristic_length: T, dy: T) -> T {
        let dt_dy_wall = (self.temperature[0][1] - self.temperature[0][0]) 
            / dy; // Use actual dy

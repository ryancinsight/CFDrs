...
impl<T: RealField + FromPrimitive> KEpsilonModel<T> {
    ...
    pub fn new(nx: usize, ny: usize, wall_function: WallFunction) -> Self {
        let k_init = T::from_f64(1e-4).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let epsilon_init = T::from_f64(1e-6).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
    ...
    pub fn update_turbulent_viscosity(&mut self) {
        let c_mu = T::from_f64(constants::C_MU).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        
        for i in 0..self.nx {
            for j in 0..self.ny {
                let k_val = self.k[i][j].clone();
                let eps_val = self.epsilon[i][j].clone().max(T::from_f64(constants::EPSILON_MIN).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?);
    ...
    fn apply_standard_wall_function(
        ...
        let kappa = T::from_f64(constants::KAPPA).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let _e_wall_function = T::from_f64(constants::E_WALL_FUNCTION).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let c_mu = T::from_f64(constants::C_MU).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        ...
        for i in 0..self.nx {
            ...
            if y_plus > T::from_f64(11.63).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? {
                // Log-law region
                ...
                // Viscous sublayer
                self.k[i][0] = T::zero();
                self.epsilon[i][0] = T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * nu.clone() * self.k[i][1].clone() / (y.clone() * y);
    ...
    fn apply_menter_sst_wall_treatment(
        ...
        let kappa = T::from_f64(constants::KAPPA).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let c_mu = T::from_f64(constants::C_MU).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let beta_star = T::from_f64(0.09).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?; // SST model constant
        ...
        for &(i_wall, j_wall) in wall_boundaries {
            ...
            let arg1 = (y_plus.clone() / T::from_f64(2.5).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?).min(T::one());
            let f1 = arg1.clone().powi(3);
            
            // k boundary condition (Menter 1994)
            if y_plus < T::from_f64(5.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? {
                // Viscous sublayer: k = 0 at wall
                self.k[i_wall][j_wall] = T::zero();
                self.k[i_near][j_near] = u_tau.clone() * u_tau.clone() * y_plus.clone() / 
                    T::from_f64(11.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
            } else {
            ...
            let beta_1 = T::from_f64(0.075).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?; // SST model constant
            
            if y_plus < T::from_f64(5.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? {
                // Viscous sublayer
                let omega_visc = T::from_f64(60.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * nu.clone() / 
            ...
            let f_mu = T::one() - (-rev.clone() / T::from_f64(70.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?).exp();
    ...
    fn calculate_friction_velocity(&self, u_p: T, y: T, nu: T) -> Result<T> {
        let kappa = T::from_f64(constants::KAPPA).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let e_wall_function = T::from_f64(constants::E_WALL_FUNCTION).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let tolerance = T::from_f64(1e-6).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let max_iter = 20;
        
        // Initial guess
        let mut u_tau = T::from_f64(0.1).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * u_p.clone();
    ...
    pub fn solve_transport_equations(
        ...
        let c1_eps = T::from_f64(constants::C1_EPSILON).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let c2_eps = T::from_f64(constants::C2_EPSILON).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let sigma_k = T::from_f64(constants::SIGMA_K).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let sigma_eps = T::from_f64(constants::SIGMA_EPSILON).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        ...
        for i in 1..self.nx-1 {
            for j in 1..self.ny-1 {
                ...
                let du_dx = (velocity[i+1][j].x.clone() - velocity[i-1][j].x.clone()) / (T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * dx.clone());
                let du_dy = (velocity[i][j+1].x.clone() - velocity[i][j-1].x.clone()) / (T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * dy.clone());
                let dv_dx = (velocity[i+1][j].y.clone() - velocity[i-1][j].y.clone()) / (T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * dx.clone());
                let dv_dy = (velocity[i][j+1].y.clone() - velocity[i][j-1].y.clone()) / (T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * dy.clone());
                
                let s11 = du_dx.clone();
                let s12 = T::from_f64(0.5).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * (du_dy.clone() + dv_dx.clone());
                let s22 = dv_dy.clone();
                
                let production = T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.nu_t[i][j].clone() * 
                    (s11.clone() * s11 + T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * s12.clone() * s12 + s22.clone() * s22);
                ...
                let destruction_coeff = c2_eps.clone() * self.epsilon[i][j].clone() / self.k[i][j].clone().max(T::from_f64(constants::EPSILON_MIN).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?);
                
                new_epsilon[i][j] = (self.epsilon[i][j].clone() + dt.clone() * source_term) / 
                                   (T::one() + dt.clone() * destruction_coeff);
                
                // Ensure positive values
                new_k[i][j] = new_k[i][j].clone().max(T::from_f64(constants::EPSILON_MIN).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?);
                new_epsilon[i][j] = new_epsilon[i][j].clone().max(T::from_f64(constants::EPSILON_MIN).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?);
            }
        }
    ...
    fn calculate_diffusion(&self, field: &[Vec<T>], i: usize, j: usize, 
        ...
        let d2f_dx2 = (field[i+1][j].clone() - T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * field[i][j].clone() 
            + field[i-1][j].clone()) / (dx.clone() * dx);
        let d2f_dy2 = (field[i][j+1].clone() - T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * field[i][j].clone() 

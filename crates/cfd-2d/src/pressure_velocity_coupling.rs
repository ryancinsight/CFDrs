...
/// Pressure–velocity coupling configuration (SIMPLE)
...
/// Pressure–velocity coupler solver (SIMPLE)
...
impl<T: RealField + FromPrimitive> PressureVelocityCouplerSolver<T> {
	...
	/// Create a new solver with fluid properties
	...
	fn apply_convection_scheme(
		...
		match self.config.convection_scheme {
			...
			SpatialScheme::SecondOrderUpwind => {
				// Hybrid scheme as a default second-order upwind proxy
			...
			SpatialScheme::Quick => {
				// Map QUICK to second-order upwind here; full QUICK is provided in finite-difference `schemes` module
				let two = T::from_f64(2.0).unwrap();
				let ae = T::max(T::max(-fe.clone(), de.clone() - fe.clone() / two.clone()), T::zero());
				let aw = T::max(T::max(fw.clone(), dw.clone() + fw.clone() / two.clone()), T::zero());
				let an = T::max(T::max(-fn_.clone(), dn.clone() - fn_.clone() / two.clone()), T::zero());
	...
	/// Explicit momentum (original implementation)
...
mod tests {
	...
	fn test_boundary_conditions() {
		...
		match result {
			Ok(_) => {}
			Err(_) => {
				// Apply BCs to ensure state is consistent

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::info!("Serpentine-Venturi 1D vs 2D Cross-Fidelity Comparison");
    tracing::info!("=====================================================\n");
    ...
    tracing::info!("Blueprint: {segments} segments, {n_venturis} venturi throats");
    tracing::info!("  Channel width:   {:.1} mm", width_m * 1e3);
    tracing::info!("  Throat width:    {:.1} mm", throat_width_m * 1e3);
    tracing::info!("  Height:          {:.1} mm", height_m * 1e3);
    tracing::info!("  Bend radius:     {:.1} mm", bend_radius_m * 1e3);
    tracing::info!("  D_h:             {:.3} mm", d_h * 1e3);
    tracing::info!(
        "  Q_inlet:         {:.2} mL/min",
        INLET_FLOW_M3_S * 60.0 * 1e6
    );
    tracing::info!("  u_mean:          {u_mean:.4} m/s");
    tracing::info!("  Re:              {re:.1}");
    tracing::info!("  Dean number:     {de:.1}");
    tracing::info!("  Contraction:     {:.1}:1", width_m / throat_width_m);
    tracing::info!(
        "  Nodes: {}  Channels: {}",
        blueprint.nodes.len(),
    ...
    tracing::info!("  Inlet pressure:  {inlet_p:.2} Pa");
    tracing::info!("  Total dP:        {inlet_p:.2} Pa (inlet to outlet = 0 Pa)");
    ...
    for edge_ref in solved.graph.edge_references() {
        ...
        let q = solved
            ...
            .get(eid.index())
            .copied()
            .map_or(edge.flow_rate.into_base(), VolumetricFlowRate::into_base);
        let p_from = solved
            .pressures
            .get(from_idx.index())
            .copied()
    ...
    tracing::info!("  Grid:            {nx} x {ny} per channel");
    tracing::info!("  Converged:       {converged}/{total}");
    tracing::info!("  Total HI:        {:.2e}", result_2d.total_hemolysis_index);
    tracing::info!(
        "  Max outlet err:  {:.2}%",
    ...
    for ch2d in &result_2d.channels {
        ...
        let (q_1d, tau_1d) = r1d.map_or((0.0, 0.0), |r| (r.flow_rate_m3_s, r.wall_shear_pa));
    ...
    .expect("invariant: report string accepts formatting");

    report.push_str("## 1D Results (cfd-1d: Casson blood)\n\n");
    writeln!(&mut report, "- Inlet pressure: {inlet_p:.2} Pa")
        .expect("invariant: report string accepts formatting");
    write!(&mut report, "- Total dP: {inlet_p:.2} Pa\n\n")
        .expect("invariant: report string accepts formatting");
    ...
    .expect("invariant: report string accepts formatting");
    writeln!(&mut report, "- Converged: {converged}/{total}")
        .expect("invariant: report string accepts formatting");
    writeln!(
        &mut report,

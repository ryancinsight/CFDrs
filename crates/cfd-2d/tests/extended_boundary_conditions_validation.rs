...
fn test_periodic_channel_flow() {
    ...
    let analytical_u = |y: f64| -> f64 { -dp_dx * y * (ly - y) / (2.0 * mu) };
    ...
    assert!(
        u_max_analytical > 0.0,
        "Poiseuille flow should have positive velocity"
    );
...
fn test_pressure_driven_flow() {
    ...
    for j in 0..ny {
        boundary_conditions.insert(
            (nx - 1, j),
            BoundaryCondition::PressureOutlet { pressure: p_out },
        );
    }
    ...
    assert!(
        dp > 0.0,
        "Pressure difference should drive flow from inlet to outlet"
    );
}

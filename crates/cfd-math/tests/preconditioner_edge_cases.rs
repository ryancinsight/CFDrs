...
fn test_ilu0_ill_conditioned_matrix() {
    ...
    ilu.apply_to(&b, &mut x).expect("Preconditioner application should succeed");
...
fn test_cholesky_non_positive_definite() {
    ...
    assert!(result.is_err(), "Cholesky should reject non-positive definite matrix");
}
...
fn test_ilu0_convergence_with_conditioning() {
    ...
    for _ in 0..10 {
        ilu.apply_to(&b, &mut x).expect("Repeated applications should succeed");
...
fn test_ilu0_extreme_values() {
    ...
    if let Ok(ilu) = result {
        ...
        if ilu.apply_to(&b, &mut x).is_ok() {
            for i in 0..n {
                assert!(x[i].is_finite(), "Solution should be finite with extreme values");
            }
        }
    }
}
...
fn test_ilu0_sparsity_preservation() {
    ...
    ilu.apply_to(&b, &mut x).expect("Application should succeed");

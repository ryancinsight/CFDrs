...
fn test_simd_add_f32() {
    ...
    simd.add(&a, &b, &mut result)
        .expect("invariant: equal-length vectors and output");
...
fn test_simd_scale_f32() {
    ...
    simd.scale(&input, scalar, &mut result)
        .expect("invariant: input and output lengths match");
...
fn test_simd_dot_f32() {
    ...
    let dot = simd
        .dot(&a, &b)
        .expect("invariant: dot-product inputs have equal length");
...
fn test_simd_sum_f32() {
    ...
    let sum = simd
        .sum_f32(&input)
        .expect("invariant: non-empty sum input");
...
fn test_simd_max_f32() {
    ...
    let max = simd
        .max_f32(&input)
        .expect("invariant: non-empty maximum input");
...
fn test_empty_vectors() {
    ...
    simd.add(&a, &b, &mut result)
        .expect("invariant: empty vectors have matching lengths");
}
...
fn test_convolution() {
    ...
    ops.convolution(&signal, &kernel, &mut result)
        .expect("invariant: convolution output has the expected length");

...
impl SpectralMesh1D {
    ...
    pub fn new(x_min: f64, x_max: f64, num_elements: usize, element_order: usize) -> Result<Self> {
        ...
        let mut global_nodes =
            Vec::with_capacity((num_elements * (element_order + 1)) - (num_elements - 1));
        ...
        for e in 0..num_elements {
            ...
            let local_nodes: Vec<f64> = element
                .nodes
                .iter()
...
mod tests {
    ...
    fn test_spectral_element() {
        let order = 4;
        let element = SpectralElement::new(order).expect("expected value");
        ...
        for i in 0..=order / 2 {
            assert_relative_eq!(element.nodes[i], -element.nodes[order - i], epsilon = 1e-10);
        }
    ...
    fn test_spectral_mesh() {
        ...
        assert_relative_eq!(
            mesh.node_coord(mesh.num_global_nodes() - 1),
            x_max,
            epsilon = 1e-10
        );

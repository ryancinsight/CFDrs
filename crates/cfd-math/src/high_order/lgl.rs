...
mod tests {
    ...
    fn lgl_nodes_order2_gives_minus1_0_plus1() {
        let nodes = lgl_nodes(2).expect("expected value");
    ...
    fn lgl_nodes_order4_symmetric() {
        let nodes = lgl_nodes(4).expect("expected value");
    ...
    fn lgl_quadrature_exact_for_polynomials() {
        ...
        for order in 2usize..=5 {
            let (nodes, weights) = lgl_nodes_and_weights(order).expect("expected value");
            let n_pts = order + 1;

...
impl StencilOps {
    ...
    pub fn laplacian_5point<T: RealField + Copy + Send + Sync>(
        ...
        for j in 1..ny - 1 {
            for i in 1..nx - 1 {
                ...
                result[idx] = (left - center * cfd_core::numeric::from_f64(2.0)?
                    + right)
                    / dx2
                    + (bottom - center * cfd_core::numeric::from_f64(2.0)? + top) / dy2;
            }
    ...
    pub fn gradient_central<T: RealField + Copy + Send + Sync>(
        ...
        let dx_inv = T::one() / (cfd_core::numeric::from_f64(2.0)? * dx);
        let dy_inv = T::one() / (cfd_core::numeric::from_f64(2.0)? * dy);
    ...
    pub fn divergence_3d<T: RealField + Copy + Send + Sync>(
        ...
        let dz_inv = T::one() / (cfd_core::numeric::from_f64(2.0)? * dz);

...
mod edge_case_tests {
    ...
    fn test_bicgstab_ill_conditioned() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 0..n {
            let diag_value = if i == 0 { 1000.0 } else if i == n - 1 { 0.001 } else { 1.0 };
    ...
    fn test_bicgstab_small_positive() {
        ...
        if let Ok(()) = result {
            ...
            assert!(result.is_err(), "Expected either success or numerical error");
        }
    }
    ...
    fn test_bicgstab_max_iterations() {
        ...
        assert!(result.is_err(), "Expected convergence failure with max_iter=1");
    }
}

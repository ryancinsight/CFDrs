...
impl<T: RealField + From<f64> + FromPrimitive + Copy> FiniteDifference<T> {
    ...
    pub fn first_derivative(&self, values: &[T]) -> Result<DVector<T>> {
        ...
        match self.scheme {
            ...
            FiniteDifferenceScheme::Central => {
                ...
                let two_inv_spacing = inv_spacing / cfd_core::numeric::from_f64(2.0)?;
    ...
    pub fn second_derivative(&self, values: &[T]) -> Result<DVector<T>> {
        ...
        result[0] = (values[2] - cfd_core::numeric::from_f64(2.0)? * values[1]
            + values[0])
            / h_squared;
        // Central difference for interior points
        for i in 1..n - 1 {
            result[i] = (values[i + 1] - cfd_core::numeric::from_f64(2.0)? * values[i]
                + values[i - 1])
                / h_squared;

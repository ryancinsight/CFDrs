...
pub fn format_report(report: &MeshAnalysisReport) -> Vec<(String, String)> {
    vec![
        ...
        ("Surface Area".to_owned(), format!("{:.6} m^2", report.surface_area)),
        ("Signed Volume".to_owned(), format!("{:.6} m^3", report.signed_volume)),
        ("Watertight".to_owned(), if report.is_watertight { "Yes" } else { "No" }.to_owned()),
    ]
}

...
use cfd_core::fluid;
use cfd_core::solver::{Configurable, Solver};
...
fn test_flow_regime_analysis() -> Result<()> {
    ...
    assert!(calculated_re < 2300.0, "Reynolds number should be in laminar regime for microfluidics");
...
fn test_surface_effects() -> Result<()> {
    ...
    assert!(expected_pressure > 0.0, "Capillary pressure should be positive for hydrophilic surface");
    
    // Test contact angle categories
    assert!(contact_angle < 90.0, "Test contact angle should be hydrophilic");
    
    // Test typical microfluidic surface tension values
    assert!(surface_tension > 0.05 && surface_tension < 0.1, "Surface tension should be in typical range for water");
...
fn test_performance_analysis() -> Result<()> {
    ...
    assert_relative_eq!(metrics.power_consumption, expected_flow_rate * pressure_drop, epsilon = 1e-15);
...
fn test_numerical_schemes() -> Result<()> {
    ...
    assert!(time_step <= max_stable_dt, "Time step violates CFL condition");
...
fn test_advanced_millifluidics() -> Result<()> {
    ...
    assert!(weber_number < 1.0, "Weber number too large for microfluidic regime");
    ...
    assert!(peclet_number > 1.0, "Convection should dominate in this regime");

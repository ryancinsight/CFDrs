...
fn test_new_components_integration() {
    ...
    builder.add_node(Node::new("inlet".to_string(), NodeType::Inlet));
    builder.add_node(Node::new("res1".to_string(), NodeType::Reservoir));
    builder.add_node(Node::new("j1".to_string(), NodeType::Junction));
    builder.add_node(Node::new("outlet".to_string(), NodeType::Outlet));
    ...
    // Verify types
    let pump = network.edge("pump1").unwrap();
    assert_eq!(pump.edge_type, EdgeType::Pump);
    
    let valve = network.edge("valve1").unwrap();
    assert_eq!(valve.edge_type, EdgeType::Valve);
    
    let res = network.node("res1").unwrap();
    assert_eq!(res.node_type, NodeType::Reservoir);
}

#[test]
fn test_from_spec_conversion() {
    use scheme::domain::model::{NodeSpec, ChannelSpec, NodeId, EdgeId, NodeKind, EdgeKind};

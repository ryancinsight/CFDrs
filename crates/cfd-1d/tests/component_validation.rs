...
fn test_micropump_parameter_validation() -> Result<()> {
    ...
    assert!(pump.efficiency <= 1.0, "Efficiency should be clamped to 1.0");
    
    pump.set_parameter("efficiency", -0.5)?;
    assert!(pump.efficiency >= 0.0, "Efficiency should be clamped to 0.0");
    ...
    assert!(pump.operating_point <= 1.0, "Operating point should be clamped");
    
    pump.set_parameter("operating_point", -0.2)?;
    assert!(pump.operating_point >= 0.0, "Operating point should be clamped");
...
fn test_flow_sensor_properties() -> Result<()> {
    ...
    assert!(resistance < r_ref * 0.01, "Sensor should have minimal resistance");

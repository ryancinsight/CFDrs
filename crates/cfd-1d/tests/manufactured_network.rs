...
fn test_series_additivity() -> Result<()> {
    ...
    let q1 = *solved.flow_rates().get(&eidx1).expect("test invariant");
    let q2 = *solved.flow_rates().get(&eidx2).expect("test invariant");
...
fn test_parallel_quadratic_branches() -> Result<()> {
    ...
    let dp = 2.0_f64;
    let q1_mag =
        ((1.0_f64 * 1.0_f64 + 4.0_f64 * 0.5_f64 * dp).sqrt() - 1.0_f64) / (2.0_f64 * 0.5_f64);
    let q2_mag =
    ...
    assert_relative_eq!(q_total, q_total_expected, max_relative = 1e-5);
...
fn test_quadratic_resistance() -> Result<()> {
    ...
    let q = *solved.flow_rates().get(&eidx1).expect("test invariant");

...
fn linearization_effective_resistance_matches_r_plus_k_abs_q() -> Result<()> {
    ...
    // This differs from Newton (R + 2k|Q_k|); both converge but Picard avoids
    // the residual offset vector required by Newton's method.
    type F = f64;
    ...
    let r_eff = 3.0_f64 + 1.5_f64 * q_abs;
...
#[test]
fn update_from_solution_picard_step_correct_sign_and_magnitude() -> Result<()> {
    // update_from_solution performs ONE Picard step (not a full solve).
    // With Q_prev = 0 (initial), R_eff = R + k*|Q_prev| = R + 0 = R, so
    //   Q = ΔP / R_eff = ΔP / R.
    // The full solve converges to the exact quadratic root after many such steps,
    // but a single call here tests the secant formula directly.
    type F = f64;
    ...
    // Single Picard step with Q_prev = 0: Q = dp / (R + k*0) = 6 / 2 = 3.0
    let dp = 6.0_f64;
    let r = 2.0_f64;
    let q_picard_step = dp / r;

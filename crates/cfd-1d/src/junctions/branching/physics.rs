...
use crate::channel::{Channel, CrossSection};
...
impl<T: RealField + Copy + FromPrimitive + ToPrimitive + SafeFromF64> TwoWayBranchJunction<T> {
    ...
    /// - Two-way branch with known geometry and fluid properties
    ...
    pub fn solve<F: FluidTrait<T> + Copy>(
        ...
        // Junction pressure error (should be small for symmetric two-way branches)
...
// ============================================================================
// Two-Way Branch Solution Result
// ============================================================================
...
// ============================================================================
// Three-Way Branch Junction Model
// ============================================================================
...
impl<T: RealField + Copy + FromPrimitive + ToPrimitive + SafeFromF64> ThreeWayBranchJunction<T> {
    ...
    /// ```
    ///
    /// Returns the relative deviation from the ideal relation.
    pub fn murray_law_deviation(&self) -> T {
        let three = T::from_f64_or_one(3.0);
...
// ============================================================================
// Three-Way Branch Solution Result
// ============================================================================
...
mod tests {
    ...
    fn test_two_way_branch_mass_conservation() {
        ...
        let two_way_branch = TwoWayBranchJunction::new(parent, d1, d2, 0.5);
    ...
    fn test_murrary_law() {
        ...
        let two_way_branch = TwoWayBranchJunction::new(parent, d1, d2, 0.5);
        let deviation = two_way_branch.murray_law_deviation();
    ...
    fn test_three_way_branch_mass_conservation() {
        let parent_geom = ChannelGeometry::<f64>::circular(1.0e-3, 120.0e-6, 1e-6);
        let parent = Channel::new(parent_geom);

        let d1_geom = ChannelGeometry::<f64>::circular(1.0e-3, 90.0e-6, 1e-6);
        let d1 = Channel::new(d1_geom);

        let d2_geom = ChannelGeometry::<f64>::circular(1.0e-3, 80.0e-6, 1e-6);
        let d2 = Channel::new(d2_geom);

        let d3_geom = ChannelGeometry::<f64>::circular(1.0e-3, 70.0e-6, 1e-6);

    ...
    fn get_shape_factor(&self) -> T {
        ...
        match &self.geometry.cross_section {
            ...
            CrossSection::Rectangular { width, height } => {
                ...
                if b <= T::zero() {
                    return T::from_f64(64.0).unwrap_or_else(|| T::zero());
                }
                let alpha = a / b;
                ...
                T::from_f64(96.0).unwrap_or_else(T::zero)
                    * (T::one()
                        - c1 * inv_a
                        + c2 * inv_a2
                        - c3 * inv_a3
            ...
            CrossSection::Elliptical {
                ...
                let two_pi = T::from_f64(2.0 * std::f64::consts::PI).unwrap_or_else(|| T::zero());
                let a = *major_axis / T::from_f64(2.0).unwrap_or_else(T::one);
                let b = *minor_axis / T::from_f64(2.0).unwrap_or_else(T::one);

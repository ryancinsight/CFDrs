...
impl<T: RealField + Copy + FromPrimitive + Float + Sum> NetworkAnalyzerOrchestrator<T> {
    ...
    pub fn new() -> Self {
        Self {
            ...
            solver: crate::solver::core::NetworkSolver::new(),
        }
    }

    /// Create analyzer with custom solver configuration
    pub fn with_solver_config(config: crate::solver::core::SolverConfig<T>) -> Self {
        Self {
            flow_analyzer: FlowAnalyzer::new(),
            pressure_analyzer: PressureAnalyzer::new(),
            resistance_analyzer: ResistanceAnalyzer::new(),
            performance_analyzer: PerformanceAnalyzer::new(),
            solver: crate::solver::core::NetworkSolver::with_config(config),
        }
    }
    ...
    pub fn analyze(
        &mut self,
        network: &mut crate::domain::network::Network<T>,
    ) -> Result<NetworkAnalysisResult<T>> {
        // Solve the network
        let problem = crate::solver::core::NetworkProblem::new(network.clone());

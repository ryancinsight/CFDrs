...
#[inline]
fn scalar_from_usize<T: FloatElement>(value: usize, context: &str) -> T {
    let narrowed = u32::try_from(value).expect(context);
    scalar::<T>(f64::from(narrowed))
}

#[inline]
fn finite_f64<T: NumericElement>(value: T, context: &str) -> Result<f64> {
    let converted = <T as NumericElement>::to_f64(value);
    if converted.is_finite() {
        Ok(converted)
    } else {
        Err(Error::InvalidConfiguration(format!(
            "{context} requires a finite Eunomia scalar conversion"
        )))
    }
}

#[inline]
fn checked_substep_count<T: NumericElement>(value: T, context: &str) -> Result<usize> {
    let converted = finite_f64(value, context)?;
    if converted <= 1.0 {
        return Ok(1);
    }

    let ceiled = converted.ceil();
    if ceiled > f64::from(u32::MAX) {
        return Err(Error::InvalidConfiguration(format!(
            "{context} exceeds the supported u32-bounded substep count"
        )));
    }

    // The range and finiteness checks above make this float-to-integer
    // conversion exact for the supported substep-count domain.
    let narrowed = ceiled as u32;
    Ok(usize::try_from(narrowed).expect("invariant: u32 substep count fits in usize"))
}

#[inline]
fn clamp_unit<T: NumericElement + PartialOrd>(value: T) -> T {
    if value < <T as NumericElement>::ZERO {
        <T as NumericElement>::ZERO
    } else if value > <T as NumericElement>::ONE {
...
impl<T: Cfd1dScalar + Copy + FloatElement> SimulationTimeConfig<T> {
    ...
    fn uniform_timepoints(duration: T, dt: T) -> Vec<T> {
        ...
        let needs_terminal = points
            .last()
            .is_none_or(|last| <T as NumericElement>::abs(*last - duration) > tolerance);
...
impl<T: Cfd1dScalar + Copy + FloatElement> Default for BloodEdgeTransportConfig<T> {
    fn default() -> Self {
        Self {
            segments_per_edge: 4,
            max_courant_number: Dimensionless::from_base(scalar::<T>(0.9)),
        }
    }
}
...
impl TransientCompositionSimulator {
    ...
    fn simulate_coupled_pressure_timepoint<T: NetworkSolveScalar, F: FluidTrait<T> + Clone>(
        ...
        Ok((
            ...
            CompositionState {
                ...
                edge_flow_rates: Self::typed_flow_rates(&reported_flow_rates),
            },
        ))
    }
    ...
    >(
        ...
        while event_cursor < composition_events.len()
            && composition_events[event_cursor].time.into_base() <= previous_time
        {
        ...
        while flow_event_cursor < flow_events.len()
            && flow_events[flow_event_cursor].time.into_base() <= previous_time
        {
            let event = &flow_events[flow_event_cursor];
            active_flow_overrides.insert(event.edge_index, event.flow_rate.into_base());
            flow_event_cursor += 1;
        }
        ...
        for &time in merged_timepoints.iter().skip(1) {
            ...
            while event_cursor < composition_events.len()
                && composition_events[event_cursor].time.into_base() <= time
            {
                let event = &composition_events[event_cursor];
                active_inlet_mixtures.insert(event.node_index, event.mixture.clone());
                event_cursor += 1;
    ...
    >(
        ...
        for (edge_index, segments) in segment_state {
            ...
            let Some(segment_mu) = Self::segment_integrated_blood_apparent_viscosity(
                props
                    .hydraulic_diameter
                    .map(|diameter| diameter.into_base()),
                flow_rate,
        ...
        if count == 0 {
            ...
            Some(
                mu_sum
                    / scalar_from_usize::<T>(
                        count,
                        "invariant: segment count fits in u32 for viscosity averaging",
                    ),
            )
        }
    }
    ...
    fn edge_supports_transport<T: Cfd1dScalar + Copy + FloatElement, F: FluidTrait<T> + Clone>(
        ...
        let tolerance = scalar::<T>(1.0e-12);
        network.properties.get(&edge_idx).is_some_and(|props| {
            props.area.into_base() > tolerance && props.length.into_base() > tolerance
        })
    }
    ...
    >(
        ...
        for edge_ref in network.graph.edge_references() {
            ...
            let residence_time = props.area.into_base() * props.length.into_base()
    ...
    >(
        ...
        for edge_ref in network.graph.edge_references() {
            ...
            let segment_count = scalar_from_usize::<T>(
                current_segments.len(),
                "invariant: segment count fits in u32 for transport substepping",
            );
            let segment_residence_time = props.area.into_base() * props.length.into_base()
                / (<T as NumericElement>::abs(flow_rate) * segment_count);
            ...
            let steps = checked_substep_count(
                total_courant / config.max_courant_number.into_base(),
                "Segmented blood transport substep count",
            )?;
            let sub_dt = dt
                / scalar_from_usize::<T>(
                    steps,
                    "invariant: substep count fits in u32 for transport substepping",
                );
    ...
    >(
        ...
        for node_idx in network.graph.node_indices() {
            ...
            if incoming.len() == 1 && outgoing.len() == 2 {
                ...
                let Some(parent_diameter) = parent_props.hydraulic_diameter.map(|d| d.into_base())
                else {
                    for &(edge_idx, _) in &outgoing {
                        edge_inlet_hematocrits.insert(edge_idx, node_hct);
                    }
                ...
                let Some(first_diameter) = first_props.hydraulic_diameter.map(|d| d.into_base())
                else {
                    for &(edge_idx, _) in &daughters {
                        edge_inlet_hematocrits.insert(edge_idx, node_hct);
                    }
                ...
                let Some(second_diameter) = second_props.hydraulic_diameter.map(|d| d.into_base())
                else {
                    for &(edge_idx, _) in &daughters {
                        edge_inlet_hematocrits.insert(edge_idx, node_hct);
                    }

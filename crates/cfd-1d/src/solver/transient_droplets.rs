//! Transient droplet tracking over solved 1D network flow fields.
//!
//! This module tracks droplet states and channel occupancy transitions:
//! `Injection -> Network -> Sink|Trapped`.

use super::transient_composition::{
    CompositionState, EdgeFlowEvent, InletCompositionEvent, MixtureComposition, PressureBoundaryEvent,
...
#[derive(Debug, Clone)]
struct ActiveDroplet<T: RealField + Copy> {
    state: DropletState,
    branches: Vec<DropletBranch<T>>,
}

#[derive(Debug, Clone)]
struct DropletBranch<T: RealField + Copy> {
    channel_index: usize,
    center: T,
    volume: T,
}

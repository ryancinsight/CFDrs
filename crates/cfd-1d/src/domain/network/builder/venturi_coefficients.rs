    ...
    F: FluidTrait<T> + Clone,
{
    ...
    let convergent_angle = validate_angle("convergent half-angle", metadata.convergent_half_angle_deg)?;
    let divergent_angle = validate_angle("divergent half-angle", metadata.divergent_half_angle_deg)?;
...
mod tests {
    ...
    fn venturi_coefficients_matches_manual_model() -> cfd_core::error::Result<()> {
        ...
        let manual = VenturiModel::new(inlet_d_t, throat_d_t, outlet_d_t, throat_len_t, total_len_t)
            .with_geometry(VenturiGeometry::Custom {
                discharge_coefficient: discharge_coefficient_from_convergent_half_angle_deg(
                    metadata.convergent_half_angle_deg,
                )?,

...
pub struct DarcyWeisbachModel<T> {
...
impl<T: CfdScalar> DarcyWeisbachModel<T> {
    ...
    fn calculate_friction_factor(&self, reynolds: T) -> T {
        ...
        let denom = c - two * b + a;
        let ba = b - a;
        let mut x = if (denom) > <T as NumericElement>::abs(T::default_epsilon()) {
            a - (ba * ba) / denom
        } else {
        ...
        for _ in 0..max_iter {
            ...
            if <T as NumericElement>::abs(diff) < tolerance {
                break;
            }
        }
...
mod tests {
    ...
    fn calculate_coefficients_physically_reasonable() {
        ...
        let (r2, k2) = model
            .calculate_coefficients(&fluid, &cond_turb)
            .expect("expected value");
        assert!(
            (r2 - 0.0).abs() < 1e-12,

...
use cfd_mesh::application::csg::boolean::{self, BooleanOp};
use cfd_mesh::domain::core::error::{MeshError, MeshResult};
...
impl BlueprintMeshPipeline {
    ...
    ///
    /// # Errors
    /// Returns `Err` if network routing, mesh generation, or CSG assembly fails.
...
fn assemble_fluid_mesh_with_policy(
    ...
    let mut accumulated = match cfd_mesh::application::csg::boolean::csg_boolean_nary(
        cfd_mesh::application::csg::BooleanOp::Union,
        &meshes,
    ) {
        Ok(mesh) => mesh,
        Err(_) if meshes.len() > 1 => {
            ...
            for mesh in iter {
                accumulated = match cfd_mesh::application::csg::boolean::csg_boolean(
                    cfd_mesh::application::csg::BooleanOp::Union,
                    &accumulated,
                    &mesh,
                ) {
                    Ok(unioned) => unioned,
                    Err(_) => cfd_mesh::application::csg::boolean::csg_boolean(
                        cfd_mesh::application::csg::BooleanOp::Union,
                        &mesh,
                        &accumulated,
                    )?,
...
fn repair_pipeline_mesh(mesh: &mut IndexedMesh, require_watertight: bool) -> MeshResult<()> {
    ...
    if !mesh.is_watertight() {
        let edge_store =
            cfd_mesh::infrastructure::storage::edge_store::EdgeStore::from_face_store(&mesh.faces);
        let added = cfd_mesh::application::watertight::seal::seal_boundary_loops(
            &mut mesh.vertices,
            &mut mesh.faces,
            &edge_store,
            RegionId::INVALID,
    ...
    if !mesh.is_watertight() {
        let improved =
            cfd_mesh::application::watertight::repair::MeshRepair::iterative_boundary_stitch(
                &mut mesh.faces,
                &mesh.vertices,
                3,
            );
...
fn build_venturi_chain_mesh(
    ...
    let mut all_faces: Vec<cfd_mesh::infrastructure::storage::face_store::FaceData> = Vec::new();
    ...
    let make_ring = |position: Point3r,
                     r: f64,
                     pool: &mut VertexPool|
     -> Vec<cfd_mesh::domain::core::index::VertexId> {
    ...
    let mut mesh = IndexedMesh::new();
    let mut vertex_remap: Vec<cfd_mesh::domain::core::index::VertexId> =

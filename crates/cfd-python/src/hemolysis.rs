//! Hemolysis models exposed to Python
...
impl PyHemolysisModel {
    ...
    fn damage_index(&self, shear_stress: f64, exposure_time: f64) -> PyResult<f64> {
        self.inner.damage_index(shear_stress, exposure_time).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!("Hemolysis calculation error: {:?}", e))
        })
    }
}

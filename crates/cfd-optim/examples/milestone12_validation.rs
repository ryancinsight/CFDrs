...
use cfd_optim::{
    pct_diff, Milestone12ReportDesign, ValidationRow, BLOOD_DENSITY_KG_M3 as RHO,
    BLOOD_VAPOR_PRESSURE_PA as P_VAPOR_PA, P_ATM_PA,
};
use serde::Serialize;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let option2_ranked =
        load_ranked_designs(&out_dir.join("two_concept_option2_venturi_top5.json"))?;
    let ga_ranked = load_ranked_designs(&out_dir.join("ga_hydrosdt_top5.json"))?;

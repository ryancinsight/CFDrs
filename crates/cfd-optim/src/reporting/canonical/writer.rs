...
use crate::reporting::types::{ValidationRow, pass_fail, pediatric_limit_pct};
...
pub fn write_milestone12_results(
    ...
    // Stream markdown directly to file to avoid ~200-500 KB in-memory string.
    let file = std::fs::File::create(path)?;
    let mut md = std::io::BufWriter::new(file);
    ...
    if validation_rows.is_empty() {
        ...
        for row in validation_rows {
            let dp_3d = row.dp_3d_fem_pa.map_or("—".to_string(), |v| format!("{v:.2}"));
            let agree_2d_3d = row.agreement_2d_3d_pct.map_or("—".to_string(), |v| format!("{v:.2}"));
            let mass_err = row.mass_error_3d_pct.map_or("—".to_string(), |v| format!("{v:.2}"));
            writeln!(
                md,
                "| {} | `{}` | {:.2} | {:.2} | {} | {:.2} | {} | {} | {} | {} |",
                row.track,

...
use super::math::{
    ...
    log_risk, mean, percentile_mut, resonance_match, split_stage_flow_fractions,
};
...
pub fn compute_blueprint_report_metrics(
    ...
    let sample_count = solve.channel_samples.len();
    let mut main_shears = Vec::with_capacity(sample_count);
    let mut main_transits = Vec::with_capacity(sample_count);
    ...
    let mut bypass_hi_values = Vec::with_capacity(sample_count);
    ...
    // Compute mean and CV *before* sorting (order-independent), then use
    // percentile_mut which sorts main_shears in-place — avoids the .to_vec()
    // heap clone inside percentile().
    let wall_shear_mean_pa = mean(&main_shears);
    let wall_shear_cv = coefficient_of_variation(&main_shears, wall_shear_mean_pa);
    // After this call main_shears is sorted — subsequent percentile_mut reuses
    // the sorted order (sort_unstable on already-sorted data is O(n)).
    let wall_shear_p95_pa = percentile_mut(&mut main_shears, 0.95);
    let wall_shear_p99_pa = percentile_mut(&mut main_shears, 0.99);
    ...
    // ── Throat-level thermal parameters ─────────────────────────────────────
    //
    // ## Theorem: Viscous Heating in Venturi Throat (Rectangular Duct)
    //
    // For steady, fully-developed Poiseuille flow through a rectangular duct
    // of width w and height h (w ≪ h), the maximum wall shear rate occurs on
    // the narrow (width) walls:
    //
    //   γ̇_wall = 6 v_mean / w                                        [s⁻¹]
    //
    // where v_mean = Q / (w × h) is the mean throat velocity.
    //
    // Wall shear stress:
    //   τ_wall = μ × γ̇_wall                                          [Pa]
    //
    // Viscous dissipation rate per unit volume (Φ = τ:∇v ≈ τ γ̇ for
    // unidirectional flow):
    //   Φ = τ × γ̇ = μ × γ̇²                                       [W/m³]
    //
    // Adiabatic temperature rise over throat transit time t_transit:
    //   ΔT = Φ t_transit / (ρ c_p)
    //      = μ γ̇² L_throat / (v_mean ρ c_p)                           [K]
    //
    // Proof of dimensional consistency:
    //   [Pa·s⁻¹·s⁻¹·m] / ([m/s]·[kg/m³]·[J/(kg·K)])
    //   = [Pa/s·m] / [J/(m²·K)]
    //   = [Pa·m·K / (J/m²)]   (since Pa = J/m³)
    //   = [K]                                                           ∎
    //
    // The previous implementation erroneously used the *channel* cross-section
    // (e.g. 8 mm wide) for venturi shear rate computation.  For a 45 µm throat
    // the correct shear rate is ~(8000/45)² ≈ 31,600× larger, producing a
    // measurable thermal rise instead of a false 0.00 K.
    //
    // Reference: Bird, Stewart & Lightfoot, "Transport Phenomena", 2nd ed.,
    //            Eq. 2.3-21 (rectangular duct profile), Ch. 10 (viscous heating).
    let mut throat_shear_rate_inv_s = 0.0_f64;
    let mut throat_shear_pa = 0.0_f64;
    let mut throat_transit_time_s = 0.0_f64;

    for (i, topo_placement) in topology.venturi_placements.iter().enumerate() {
        let tg = &topo_placement.throat_geometry;

        // Use the effective throat velocity from the venturi solver if available
        // (it accounts for the vena contracta coefficient); otherwise fall back
        // to continuity: v_throat = Q / A_throat.
        let throat_velocity_m_s = venturi.placements.get(i).map_or_else(
            || {
                let throat_area_m2 = (tg.throat_width_m * tg.throat_height_m).max(1.0e-18);
                solve
                    .channel_samples
                    .iter()
                    .find(|s| {
                        s.id == topo_placement.target_channel_id
                            || s.id.starts_with(&topo_placement.target_channel_id)
                    })
                    .map_or(0.0, |s| s.flow_m3_s.abs() / throat_area_m2)
            },
            |vm| vm.effective_throat_velocity_m_s,
        );

        // Wall shear rate on narrow walls: γ̇ = 6v / min(w, h).
        let min_dim = tg.throat_width_m.min(tg.throat_height_m).max(1.0e-18);
        let local_shear_rate = 6.0 * throat_velocity_m_s / min_dim;
        let local_shear_pa = BLOOD_VISCOSITY_PA_S * local_shear_rate;
        let local_transit = tg.throat_length_m / throat_velocity_m_s.max(1.0e-18);

        throat_shear_rate_inv_s = throat_shear_rate_inv_s.max(local_shear_rate);
        throat_shear_pa = throat_shear_pa.max(local_shear_pa);
        throat_transit_time_s = throat_transit_time_s.max(local_transit);
    }

    let throat_temperature_rise_k = if throat_shear_rate_inv_s > 0.0 {
        throat_shear_pa * throat_shear_rate_inv_s * throat_transit_time_s
    ...
    metrics.throat_shear_rate_inv_s = throat_shear_rate_inv_s;
    metrics.throat_shear_pa = throat_shear_pa;
    metrics.throat_exceeds_fda = throat_shear_pa > FDA_MAX_WALL_SHEAR_PA;
    ...
    metrics.treatment_zone_mode = match topology.treatment_mode {
        TreatmentActuationMode::UltrasoundOnly => crate::metrics::TreatmentZoneMode::UltrasoundOnly,
        TreatmentActuationMode::VenturiCavitation => crate::metrics::TreatmentZoneMode::VenturiThroats,
    };
    ...
    metrics.throat_transit_time_s = throat_transit_time_s;
    metrics.fda_overall_compliant = metrics.fda_main_compliant
        && (throat_shear_pa <= FDA_MAX_WALL_SHEAR_PA
            || (throat_transit_time_s <= FDA_TRANSIENT_TIME_S
                && throat_shear_pa <= FDA_TRANSIENT_SHEAR_PA));
    ...
    metrics.acoustic_capture_efficiency =
        (cavitation_potential * (throat_shear_pa / P_ATM_PA)).clamp(0.0, 1.0);
...
mod tests {
    ...
    use super::compute_blueprint_report_metrics;

    /// Verify that a venturi candidate with active throat placements produces
    /// non-zero throat thermal rise.  Before the fix, the computation used the
    /// channel cross-section (e.g. 2 mm wide) instead of the throat geometry
    /// (0.4 mm wide), underestimating shear rate and yielding 0.00 K.
    #[test]
    fn throat_temperature_rise_is_nonzero_for_active_venturi() {
        // canonical_option2_candidate: 2-stage trifurcation + bifurcation with
        // 2× serial venturi throats (throat_width = 400 µm, height = 1.1 mm).
        // Flow: 2 µL/s ≈ 0.12 mL/min, 30 kPa gauge.
        ...
        // With active venturi throats, throat shear rate must be positive.
        assert!(
            metrics.throat_shear_rate_inv_s > 0.0,
            "throat_shear_rate_inv_s = {} (expected > 0 for active venturi)",
            metrics.throat_shear_rate_inv_s
        );

        // Throat shear stress must be positive.
        assert!(
            metrics.throat_shear_pa > 0.0,
            "throat_shear_pa = {} (expected > 0 for active venturi)",
            metrics.throat_shear_pa
        );

        // Thermal rise must be non-zero.  For a 400 µm throat at even modest
        // flow, viscous dissipation produces measurable heating.
        assert!(
            metrics.throat_temperature_rise_k > 0.0,
            "throat_temperature_rise_k = {} K (expected > 0 for active venturi)",
            metrics.throat_temperature_rise_k
        );

        // Sanity: thermal rise should be well below the FDA 5 K limit at this
        // modest flow rate.
        assert!(
            metrics.throat_temperature_rise_k < 5.0,
            "throat_temperature_rise_k = {} K exceeds FDA 5 K limit (unexpected at low flow)",
            metrics.throat_temperature_rise_k
        );
    }
}

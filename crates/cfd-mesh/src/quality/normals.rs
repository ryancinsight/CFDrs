...
impl NormalAnalysis {
    ...
    pub fn inward_fraction(&self) -> Real {
        ...
        if n > 0.0 { self.inward_faces as Real / n } else { 0.0 }
    }
...
/// use cfd_mesh::quality::normals::analyze_normals;
///
...
mod tests {
    use super::*;
    use crate::geometry::primitives::{PrimitiveMesh, UvSphere, Cube};
    use crate::core::scalar::Point3r;

    #[test]
    fn sphere_all_outward() {
        let mesh = UvSphere { radius: 1.0, segments: 32, stacks: 16, ..Default::default() }
            .build()
            .unwrap();
        let r = analyze_normals(&mesh);
        assert_eq!(r.inward_faces, 0, "UV sphere should have zero inward faces");
        assert_eq!(r.degenerate_faces, 0, "UV sphere should have no degenerate faces");
        assert!(r.face_vertex_alignment_mean > 0.9,
            "face-vertex alignment mean should be > 0.9, got {}", r.face_vertex_alignment_mean);
    }
    ...
    fn inward_fraction_zero_on_clean_mesh() {
        let mesh = UvSphere { radius: 1.0, segments: 16, stacks: 8, ..Default::default() }
            .build()
            .unwrap();
        let r = analyze_normals(&mesh);

...
fn long_constraint_through_random_cloud() {
    ...
    let n = 40;
    for i in 0..n {
        let px = 1.0 + (i as f64) * 8.0 / (n as f64);
        let py = if i % 2 == 0 { 1.2 + (i as f64) * 0.02 } else { 3.5 + (i as f64) * 0.02 };
...
fn ruppert_area_only_refinement() {
    ...
    use crate::application::delaunay::refinement::ruppert::RuppertRefiner;
...
fn cdt_zigzag_constraint() {
    ...
    for i in 1..=n_zigs {
        let x = 1.0 + (i as f64) * 18.0 / (n_zigs as f64);

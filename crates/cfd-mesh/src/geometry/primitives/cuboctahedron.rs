...
fn build(co: &Cuboctahedron) -> Result<IndexedMesh, PrimitiveError> {
    if co.radius <= 0.0 {
        return Err(PrimitiveError::InvalidParam(format!(
            "radius must be > 0, got {}", co.radius
        )));
    }
    ...
    for [ia, ib, ic] in TRI_FACES {
        ...
        let n = match raw_n {
            Some(n) => if n.dot(&centroid) < 0.0 { -n } else { n },
            None => Vector3r::zeros(),
        };
    ...
    for [ia, ib, ic, id] in QUAD_FACES {
        let centroid = (verts[ia].coords + verts[ib].coords + verts[ic].coords + verts[id].coords) / 4.0;
        // Triangle 1: ia, ib, ic
        {
            let raw_n = triangle_normal(&verts[ia], &verts[ib], &verts[ic]);
            let n = match raw_n {
...
mod tests {
    ...
    fn cuboctahedron_is_watertight() {
        ...
        assert!(report.is_watertight, "cuboctahedron must be watertight: {:?}", report);
        assert_eq!(report.euler_characteristic, Some(2));
    }

    #[test]
    fn cuboctahedron_volume_positive() {
        let mesh = Cuboctahedron { radius: 2.0, ..Cuboctahedron::default() }.build().unwrap();
        let edges = EdgeStore::from_face_store(&mesh.faces);
        let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    ...
    fn cuboctahedron_invalid_radius() {
        assert!(Cuboctahedron { radius: 0.0, ..Cuboctahedron::default() }.build().is_err());
        assert!(Cuboctahedron { radius: -1.0, ..Cuboctahedron::default() }.build().is_err());
    }
}

...
impl<T: Scalar> VertexData<T> {
    ...
    pub fn from_position(position: Point3<T>) -> Self {
        Self { position, normal: Vector3::zeros() }
    }
    ...
    pub fn lerp(&self, other: &Self, t: T) -> Self {
        ...
        let position = Point3::from(
            self.position.coords * one_minus_t + other.position.coords * t,
        );
        let n = self.normal * one_minus_t + other.normal * t;
        let len = n.norm();
        let normal = if len > T::zero() { n / len } else { Vector3::zeros() };
        Self { position, normal }
    }
}
...
impl<T: Scalar> VertexPool<T> {
    ...
    pub fn insert_or_weld(&mut self, position: Point3<T>, normal: Vector3<T>) -> VertexId {
        ...
        for dz in -1i64..=1 {
            for dy in -1i64..=1 {
                for dx in -1i64..=1 {
                    let neighbour = CellKey { x: key.x + dx, y: key.y + dy, z: key.z + dz };
                    if let Some(indices) = self.spatial_hash.get(&neighbour) {
                        for &idx in indices {
                            let dist_sq = (self.vertices[idx as usize].position - position)
                                .norm_squared();
        ...
        self.spatial_hash.entry(key).or_insert_with(|| Vec::with_capacity(2)).push(idx);
        VertexId::new(idx)
    }
    ...
    #[inline]
    pub fn get(&self, id: VertexId) -> &VertexData<T> { &self.vertices[id.as_usize()] }
    ...
    pub fn iter(&self) -> impl Iterator<Item = (VertexId, &VertexData<T>)> {
        self.vertices.iter().enumerate().map(|(i, v)| (VertexId::new(i as u32), v))
    }

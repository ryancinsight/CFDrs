...
//!
//! These types provide the storage layer for [`crate::mesh::HalfEdgeMesh`].
...
//!   [`GhostCell<'id, T>`][crate::permission::GhostCell] costs no memory or
...
//! one call to [`crate::mesh::with_mesh`] are serialized through the token.

use slotmap::{SlotMap, Key};
use crate::permission::GhostCell;
...
impl<K: Key, V> SlotPool<K, V> {
    ...
    pub fn with_capacity(cap: usize) -> Self {
        Self { inner: SlotMap::with_capacity_and_key(cap) }
    }
...
///
/// This is the pool type used inside [`crate::mesh::HalfEdgeMesh`].  All
/// mutation requires a `&mut GhostToken<'id>`; all reads require a
/// `&GhostToken<'id>`.  The brand `'id` is bound by [`crate::mesh::with_mesh`]
...
///     let key = pool.insert(cfd_mesh::permission::GhostCell::new(data));
///
...
mod tests {
    ...
    fn ghost_slot_pool_borrow() {
        use crate::permission::{GhostCell, GhostToken};

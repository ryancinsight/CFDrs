...
fn build(ico: &Icosahedron) -> Result<IndexedMesh, PrimitiveError> {
    if ico.radius <= 0.0 {
        return Err(PrimitiveError::InvalidParam(format!(
            "radius must be > 0, got {}", ico.radius
        )));
    }
    ...
    let pts: [Point3r; 12] = std::array::from_fn(|i| {
        Point3r::new(cx + raw[i][0], cy + raw[i][1], cz + raw[i][2])
    });

    for [a, b, c] in ICOSAHEDRON_FACES {
        let n = triangle_normal(&pts[a], &pts[b], &pts[c])
...
mod tests {
    ...
    fn icosahedron_volume_positive_and_approximately_correct() {
        let r = 1.0_f64;
        let mesh = Icosahedron { radius: r, ..Icosahedron::default() }.build().unwrap();
        let edges = EdgeStore::from_face_store(&mesh.faces);
        let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    ...
    fn icosahedron_invalid_radius() {
        assert!(Icosahedron { radius: 0.0, ..Icosahedron::default() }.build().is_err());
        assert!(Icosahedron { radius: -1.0, ..Icosahedron::default() }.build().is_err());
    }

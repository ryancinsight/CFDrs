...
fn unit_cube_faces() -> (VertexPool, Vec<FaceData>) {
    ...
    (pool, vec![
        f(c000,c010,c110), f(c000,c110,c100), f(c001,c101,c111), f(c001,c111,c011),
        f(c000,c001,c011), f(c000,c011,c010), f(c100,c110,c111), f(c100,c111,c101),
        f(c000,c100,c101), f(c000,c101,c001), f(c010,c011,c111), f(c010,c111,c110),
    ])
}

fn build_sphere_faces(stacks: usize, segments: usize) -> (VertexPool, Vec<FaceData>) {
    let mesh = UvSphere { center: Point3r::origin(), radius: 1.0, stacks, segments }
        .build().expect("sphere");
    let mut pool = VertexPool::default_millifluidic();
    let mut id_map = std::collections::HashMap::new();
    for (old, _) in mesh.vertices.iter() {
        id_map.insert(old, pool.insert_or_weld(*mesh.vertices.position(old), *mesh.vertices.normal(old)));
    }
    let faces = mesh.faces.iter()
...
fn bench_csg_union_cube_cube(c: &mut Criterion) {
    let cube_a = Cube { origin: Point3r::new(-1.0,-1.0,-1.0), width: 2.0, height: 2.0, depth: 2.0 }.build().unwrap();
    let cube_b = Cube { origin: Point3r::new(-0.5,-0.5,-0.5), width: 2.0, height: 2.0, depth: 2.0 }.build().unwrap();
    c.bench_function("csg_union_cube_cube", |b| {
        b.iter(|| csg_boolean_indexed(BooleanOp::Union, black_box(&cube_a), black_box(&cube_b)).ok())
    });
}

fn bench_csg_intersection_cylinders(c: &mut Criterion) {
    let cyl_a = Cylinder { base_center: Point3r::new(-2.0,0.0,0.0), radius: 1.0, height: 4.0, segments: 32 }.build().unwrap();
    let cyl_b = Cylinder { base_center: Point3r::new(0.0,-2.0,0.0), radius: 1.0, height: 4.0, segments: 32 }.build().unwrap();
    c.bench_function("csg_intersection_cyl32", |b| {
        b.iter(|| csg_boolean_indexed(BooleanOp::Intersection, black_box(&cyl_a), black_box(&cyl_b)).ok())
    });
}
...
fn bench_detect_self_intersect_flat(c: &mut Criterion) {
    ...
    let n = nalgebra::Vector3::zeros();
    let ns = 10_usize;
    let mut verts = vec![vec![cfd_mesh::domain::core::index::VertexId::new(0); ns+1]; ns+1];
...
criterion_group!(gwn_benches, bench_gwn_linear_small, bench_gwn_linear_large, bench_gwn_bvh_large, bench_classify_prepared);
criterion_group!(csg_benches, bench_csg_union_cube_cube, bench_csg_intersection_cylinders);

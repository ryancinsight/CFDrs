//! CSG Diagnostic: Offset overlapping cubes with normal-orientation analysis.
//!
//! Cube A: 2×2×2 at (0,0,0)
//! Cube B: 2×2×2 at (0.5,0.5,0.5)
//! Overlap: 1.5×1.5×1.5 = 3.375 mm³
//!
//! Expected volumes:
//! - Union:        12.625 mm³
//! - Intersection:  3.375 mm³
//! - Difference:    4.625 mm³
//!
//! Run with:
//! cargo run --manifest-path CFDrs/Cargo.toml -p cfd-mesh --example csg_diagnostic_offset_cubes_normals --features "csg,stl-io"
...
const EXPECTED_UNION_VOLUME: Real = 12.625;
const EXPECTED_INTERSECTION_VOLUME: Real = 3.375;
const EXPECTED_DIFFERENCE_VOLUME: Real = 4.625;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=================================================================");
    println!("  CSG Diagnostic: Offset Cubes + Normal Analysis");
    println!("=================================================================");
    println!("  Cube A: 2×2×2 mm @ (0,0,0)");
    println!("  Cube B: 2×2×2 mm @ (0.5,0.5,0.5)");
    println!("  Overlap volume: 3.375 mm³");

    let crate_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
    let out_dir = crate_dir
        .join("outputs")
        .join("csg_diagnostic_offset_cubes_normals");

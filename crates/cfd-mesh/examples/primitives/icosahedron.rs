//! Export: Icosahedron → outputs/primitives/icosahedron.stl
use std::fs;
use std::io::BufWriter;
use cfd_mesh::Icosahedron;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mesh = Icosahedron::default().build()?;
    let edges = EdgeStore::from_face_store(&mesh.faces);
    let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    println!("Icosahedron V={} F={} watertight={} vol={:.4} chi={:?}",
    ...
    stl::write_binary_stl(&mut BufWriter::new(fs::File::create(out.join("icosahedron.stl"))?),

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mesh = StadiumPrism::default().build()?;
    let edges = EdgeStore::from_face_store(&mesh.faces);
    let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    println!("StadiumPrism V={} F={} watertight={} vol={:.4} chi={:?}",
    ...
    stl::write_binary_stl(&mut BufWriter::new(fs::File::create(out.join("stadium_prism.stl"))?),

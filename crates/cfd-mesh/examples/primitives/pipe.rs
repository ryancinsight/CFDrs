//! Export: Pipe → outputs/primitives/pipe.stl
use std::fs;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mesh = Pipe::default().build()?;
    let edges = EdgeStore::from_face_store(&mesh.faces);
    let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    println!("Pipe V={} F={} watertight={} vol={:.4} chi={:?}",
    ...
    stl::write_binary_stl(&mut BufWriter::new(fs::File::create(out.join("pipe.stl"))?),

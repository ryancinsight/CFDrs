//! Export: Cone → `outputs/primitives/cone.stl`
//!
//! Builds a closed right circular cone (r=1 mm, h=2 mm, 32 segments) using
//! [`cfd_mesh::Cone`], validates watertightness, and exports binary STL.
//!
//! Run with:
//! ```sh
//! cargo run -p cfd-mesh --example primitives_cone
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("=================================================================");
    println!("  Primitive: Cone r=1 mm, h=2 mm  (via Cone builder)");
    println!("=================================================================");

    let mesh = Cone { radius: 1.0, height: 2.0, segments: 32, ..Cone::default() }
        .build()?;

    let edges = EdgeStore::from_face_store(&mesh.faces);
    let report = check_watertight(&mesh.vertices, &mesh.faces, &edges);
    let expected_vol = PI * 1.0_f64 * 2.0 / 3.0;
    ...
    let stl_path = out_dir.join("cone.stl");

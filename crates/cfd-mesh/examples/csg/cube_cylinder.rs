...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Mesh built: {} cube + {} cylinder faces  ({} ms)",
        cube.face_count(), cylinder.face_count(), t_build.elapsed().as_millis());
    println!();
    ...
    // ── Union ─────────────────────────────────────────────────────────────────
    {
        ...
        report("Union (A ∪ B)", &mut result, v_cube + v_cyl - v_overlap, 0.05, ms);

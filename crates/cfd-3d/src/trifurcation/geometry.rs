    ...
    TrifurcationGeometry3D<T>
{
    ...
    pub fn symmetric(
        ...
        Self {
            ...
            branching_angles: [spread_angle, scalar::zero::<T>(), -spread_angle],
        }
    }

    /// Verification based on Generalized Murray's Law: D_0^3 = Σ D_i^3
    pub fn murray_law_deviation(&self) -> T {
        let sum_daughters = scalar::powi(self.d_daughters[0], 3)
            + scalar::powi(self.d_daughters[1], 3)
            + scalar::powi(self.d_daughters[2], 3);
        scalar::powi(self.d_parent, 3) - sum_daughters
    }
}
    ...
    cfd_mesh::application::delaunay::dim3::sdf::Sdf3D<T> for TrifurcationGeometry3D<T>
{
    fn eval(&self, p: &leto::Point3<T>) -> T {
        ...
        for i in 0..3 {
            ...
            let dir = Vector3::new(scalar::cos(angle), scalar::sin(angle), scalar::zero::<T>());
            let daughter = FiniteCylinderSdf::new(
                ...
                Point3::new(
                    ...
                    scalar::zero::<T>(),
                    scalar::zero::<T>(),
                ) + dir * self.l_daughters[i],
                self.d_daughters[i] / half,
            );
            min_val = scalar::min(min_val, daughter.eval(p));
        }
    ...
    fn bounds(&self) -> (leto::Point3<T>, leto::Point3<T>) {
        ...
        for i in 0..3 {
            ...
            min_pt = Point3::new(
                scalar::min(min_pt.x, d_min.x),
                scalar::min(min_pt.y, d_min.y),
                scalar::min(min_pt.z, d_min.z),
            );
            max_pt = Point3::new(
                scalar::max(max_pt.x, d_max.x),
                scalar::max(max_pt.y, d_max.y),
                scalar::max(max_pt.z, d_max.z),
            );
        }

...
impl<T: cfd_mesh::domain::core::Scalar + RealField + Copy + FromPrimitive> DynamicSmagorinskyModel<T> {
    pub fn new() -> Self {
        Self { filter_width: T::one(),
               test_filter_ratio: <T as FromPrimitive>::from_f64(2.0).unwrap_or_else(T::one),
               cs_sq_max: <T as FromPrimitive>::from_f64(DYNAMIC_CS_SQ_MAX).unwrap_or_else(T::one) }
    }
    ...
    fn test_filter(&self, flow: &FlowField<T>) -> Vec<nalgebra::Vector3<T>> {
        ...
        let eighth = <T as FromPrimitive>::from_f64(1.0 / 8.0).unwrap_or_else(T::one);
...
impl<T: cfd_mesh::domain::core::Scalar + RealField + Copy + FromPrimitive> TurbulenceModel<T> for DynamicSmagorinskyModel<T> {
    fn turbulent_viscosity(&self, flow_field: &FlowField<T>) -> Vec<T> {
        ...
        let eps = <T as FromPrimitive>::from_f64(1e-20).unwrap_or_else(T::zero);
        ...
        for k in 0..nz { for j in 0..ny { for i in 0..nx {
            ...
            let cs_sq_static = <T as FromPrimitive>::from_f64(0.01).unwrap_or_else(T::zero);
            let _ = u_hat[idx];
        ...
        let cs_sq_avg = if n > 0 {
            ...
            sum / <T as FromPrimitive>::from_f64(n as f64).unwrap_or_else(T::one)
        } else { T::zero() };

    ...
    for SpalartAllmarasModel<T>
{
    ...
    fn turbulent_viscosity(&self, flow_field: &FlowField<T>) -> Vec<T> {
        ...
        for idx in 0..n {
            let nu_t = if idx < self.nu_tilde.len() { self.nu_tilde[idx] } else { T::zero() };
    ...
    fn name(&self) -> &'static str { "Spalart-Allmaras" }
}

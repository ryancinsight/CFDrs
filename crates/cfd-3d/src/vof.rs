...
impl<T: RealField + FromPrimitive + Copy> VofSolver<T> {
    ...
    pub fn new(config: VofConfig, nx: usize, ny: usize, nz: usize, dx: T, dy: T, dz: T) -> Self {
    ...
    pub fn initialize_sphere(&mut self, center: Vector3<T>, radius: T) {
        for k in 0..self.nz {
            for j in 0..self.ny {
                for i in 0..self.nx {
                    let x = (T::from_usize(i).unwrap_or_else(|| T::zero())
                        + T::from_f64(0.5).unwrap_or_else(|| T::zero()))
                        * self.dx;
                    let y = (T::from_usize(j).unwrap_or_else(|| T::zero())
                        + T::from_f64(0.5).unwrap_or_else(|| T::zero()))
                        * self.dy;
                    let z = (T::from_usize(k).unwrap_or_else(|| T::zero())
                        + T::from_f64(0.5).unwrap_or_else(|| T::zero()))
                        * self.dz;
                    ...
                    let eps =
                        T::from_f64(INTERFACE_THICKNESS).unwrap_or_else(|| T::zero()) * self.dx;
                    let arg = (radius - distance) / eps;
                    self.alpha[idx] =
                        T::from_f64(0.5).unwrap_or_else(|| T::zero()) * (T::one() + arg.tanh());
                }
            }
        }
            ...
            for j in 2..self.ny - 2 {
                for i in 2..self.nx - 2 {
                    ...
                    if *alpha_val > T::from_f64(VOF_INTERFACE_LOWER).unwrap_or_else(|| T::zero())
                        && *alpha_val
                            < T::from_f64(VOF_INTERFACE_UPPER).unwrap_or_else(|| T::zero())
                    {
                        ...
                        let dn_dx = (self.normals[self.index(i + 1, j, k)].x
                            - self.normals[self.index(i - 1, j, k)].x)
                            / two_dx;
                        let dn_dy = (self.normals[self.index(i, j + 1, k)].y
                            - self.normals[self.index(i, j - 1, k)].y)
                            / two_dy;
                        let dn_dz = (self.normals[self.index(i, j, k + 1)].z
                            - self.normals[self.index(i, j, k - 1)].z)
                            / two_dz;
    ...
    fn plic_reconstruction(&self, i: usize, j: usize, k: usize) -> (Vector3<T>, T) {
        ...
        if normal.norm() > T::from_f64(VOF_EPSILON).unwrap_or_else(|| T::zero()) {
            ...
            // Binary search for d
            let mut d_min =
                -normal[0].abs() * self.dx - normal[1].abs() * self.dy - normal[2].abs() * self.dz;
            let mut d_max = -d_min;
            ...
            for _ in 0..constants::PLIC_MAX_ITERATIONS {
                ...
                if (volume - target_volume).abs()
                    < T::from_f64(self.config.tolerance).unwrap_or_else(|| T::zero()) * cell_volume
                {
                    break;
                }
    ...
    fn calculate_volume_under_plane(
        &self,
        normal: &Vector3<T>,
        d: T,
        i: usize,
        ...
        let max_dist =
            (nx.abs() + ny.abs() + nz.abs()) * T::from_f64(0.5).unwrap_or_else(|| T::zero());
        ...
        let volume_fraction =
            (T::one() - center_dist / max_dist) * T::from_f64(0.5).unwrap_or_else(|| T::zero());
            ...
            for j in 1..self.ny - 1 {
                for i in 1..self.nx - 1 {
                    ...
                    alpha_new[idx] = self.alpha_previous[idx]
                        - dt / cell_volume
                            * (flux_x_plus - flux_x_minus + flux_y_plus - flux_y_minus
                                + flux_z_plus
                                - flux_z_minus);
...
mod tests {
    ...
    fn test_vof_creation() {
        ...
        let solver: VofSolver<f64> = VofSolver::new(config, 10, 10, 10, 0.1, 0.1, 0.1);
    ...
    fn test_sphere_initialization() {
        let config = VofConfig::default();
        let mut solver: VofSolver<f64> = VofSolver::new(config, 20, 20, 20, 0.05, 0.05, 0.05);
        ...
        assert!(
            (total_vol - sphere_vol).abs() / sphere_vol < 0.5,
            "Volume mismatch: total={}, expected={}",
            total_vol,
            sphere_vol
        );
    }
    ...
    fn test_mass_conservation() {
        let config = VofConfig::default();
        let mut solver: VofSolver<f64> = VofSolver::new(config, 10, 10, 10, 0.1, 0.1, 0.1);

        solver.initialize_block(Vector3::new(0.2, 0.2, 0.2), Vector3::new(0.8, 0.8, 0.8));

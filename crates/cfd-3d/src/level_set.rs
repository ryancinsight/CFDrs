...
impl<T: RealField + FromPrimitive> LevelSetSolver<T> {
    ...
    pub fn initialize_sphere(&mut self, center: Vector3<T>, radius: T) {
        for k in 0..self.nz {
            for j in 0..self.ny {
                for i in 0..self.nx {
                    let x = T::from_usize(i).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dx.clone();
                    let y = T::from_usize(j).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dy.clone();
                    let z = T::from_usize(k).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dz.clone();
    ...
    fn update_narrow_band(&mut self) {
        ...
        let band_width = T::from_f64(self.config.band_width).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? 
    ...
    fn weno5_derivative(&self, v: &[T], direction: usize, i: usize, j: usize, k: usize) -> (T, T) {
        let idx = self.index(i, j, k);
        let eps = T::from_f64(1e-6).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        ...
        let six = T::from_f64(6.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        ...
        let beta0 = (thirteen.clone() * (vm2.clone() - two.clone() * vm1.clone() + v0.clone()).powi(2)
            + three.clone() * (vm2.clone() - T::from_f64(4.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * vm1.clone() + three.clone() * v0.clone()).powi(2)) / T::from_f64(12.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let beta1 = (thirteen.clone() * (vm1.clone() - two.clone() * v0.clone() + vp1.clone()).powi(2)
            + three.clone() * (vm1.clone() - vp1.clone()).powi(2)) / T::from_f64(12.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        let beta2 = (thirteen.clone() * (v0.clone() - two.clone() * vp1.clone() + vp2.clone()).powi(2)
            + three.clone() * (three.clone() * v0.clone() - T::from_f64(4.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * vp1.clone() + vp2.clone()).powi(2)) / T::from_f64(12.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?;
        ...
        let derivative_minus = (w0.clone() * (two.clone() * vm1.clone() - T::from_f64(7.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * v0.clone() + T::from_f64(11.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * vp1.clone())
            + w1.clone() * (-vm1.clone() + T::from_f64(5.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * v0.clone() + two.clone() * vp1.clone())
            + w2.clone() * (two.clone() * v0.clone() + T::from_f64(5.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * vp1.clone() - vp2.clone())) / (six.clone() * h.clone());
        
        // Positive-biased stencil (for negative velocities)
        // Need to recalculate with shifted stencil
        let derivative_plus = (w2 * (T::from_f64(11.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * v0.clone() - T::from_f64(7.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * vp1.clone() + two.clone() * vp2)
            + w1 * (two.clone() * vm1.clone() + T::from_f64(5.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * v0.clone() - vp1.clone())
            + w0 * (-vm2.clone() + T::from_f64(5.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * vm1 + two * v0)) / (six * h);
    ...
    pub fn advect(&mut self, dt: T) {
        ...
        if cfl > T::from_f64(self.config.cfl_number).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? {
            // Warning: CFL condition violated, stability may be compromised
            // In production, should adapt time step or use sub-stepping
            let recommended_dt = T::from_f64(self.config.cfl_number).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * min_spacing / max_velocity;
    ...
    pub fn reinitialize(&mut self) {
        ...
        let epsilon = T::from_f64(EPSILON_SMOOTHING).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dx.clone();
        let sign_phi = self.phi.iter()
            .map(|p| self.smooth_sign(p.clone(), epsilon.clone()))
            .collect::<Vec<_>>();
        
        let dtau = T::from_f64(0.5).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dx.clone().min(self.dy.clone()).min(self.dz.clone());
        ...
        for _ in 0..self.config.max_iterations {
            ...
            if error < T::from_f64(self.config.tolerance).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? {
                break;
            }
        }
        ...
        for k in 1..self.nz-1 {
            for j in 1..self.ny-1 {
                for i in 1..self.nx-1 {
                    ...
                    // Second derivatives
                    let phi_xx = (self.phi[self.index(i+1, j, k)].clone() - T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.phi[idx].clone() 
                        + self.phi[self.index(i-1, j, k)].clone()) / (self.dx.clone() * self.dx.clone());
                    let phi_yy = (self.phi[self.index(i, j+1, k)].clone() - T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.phi[idx].clone() 
                        + self.phi[self.index(i, j-1, k)].clone()) / (self.dy.clone() * self.dy.clone());
                    let phi_zz = (self.phi[self.index(i, j, k+1)].clone() - T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.phi[idx].clone() 
                    ...
                    let phi_xy = (self.phi[self.index(i+1, j+1, k)].clone() - self.phi[self.index(i+1, j-1, k)].clone()
                        ...
                        / (T::from_f64(4.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dx.clone() * self.dy.clone());
                    let phi_xz = (self.phi[self.index(i+1, j, k+1)].clone() - self.phi[self.index(i+1, j, k-1)].clone()
                        ...
                        / (T::from_f64(4.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dx.clone() * self.dz.clone());
                    let phi_yz = (self.phi[self.index(i, j+1, k+1)].clone() - self.phi[self.index(i, j+1, k-1)].clone()
                        ...
                        / (T::from_f64(4.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dy.clone() * self.dz.clone());
                    
                    // Compute curvature
                    let grad_norm_sq = phi_x.clone() * phi_x.clone() + phi_y.clone() * phi_y.clone() + phi_z.clone() * phi_z.clone();
                    let grad_norm = ComplexField::sqrt(grad_norm_sq.clone() + T::from_f64(1e-10).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?);
                    ...
                    curvature[idx] = (phi_xx.clone() * (phi_y.clone() * phi_y.clone() + phi_z.clone() * phi_z.clone())
                        ...
                        - T::from_f64(2.0).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * (phi_x.clone() * phi_y.clone() * phi_xy 
    ...
    pub fn get_volume_fraction(&self) -> Vec<T> {
        self.phi.iter()
            .map(|p| {
                let p = p.clone();
                let eps = T::from_f64(EPSILON_SMOOTHING).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * self.dx.clone();
                if p.clone() > eps.clone() {
                    T::zero()
                } else if p.clone() < -eps.clone() {
                    T::one()
                } else {
                    T::from_f64(0.5).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * (T::one() - p.clone() / eps.clone() 
                        - ComplexField::sin(T::from_f64(std::f64::consts::PI).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? * p / eps) 
                        / T::from_f64(std::f64::consts::PI).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?)
                }
            })
            .collect()
    }
    ...
    pub fn step(&mut self, dt: T) -> Result<()> {
        ...
        let dt_cfl = T::from_f64(self.config.cfl_number).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))? 
            * self.dx.clone().min(self.dy.clone()).min(self.dz.clone()) / (max_vel + T::from_f64(1e-10).ok_or_else(|| cfd_core::error::Error::Numerical(cfd_core::error::NumericalErrorKind::InvalidFpOperation))?);

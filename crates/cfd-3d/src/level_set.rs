...
impl<T: RealField + FromPrimitive + Copy> LevelSetSolver<T> {
    ...
    fn weno5_derivative(&self, v: &[T], direction: usize, i: usize, j: usize, k: usize) -> (T, T) {
        let idx = self.index(i, j, k);
        let eps = T::from_f64(1e-6).unwrap_or_else(|| T::zero());
        ...
        let (vm2, vm1, v0, vp1, vp2, _vp3) = match direction {
            0 => {
                ...
                if i < 2 || i >= self.nx - 3 {
                    ...
                    let vp1 = if i < self.nx - 1 {
                        v[self.index(i + 1, j, k)]
                    } else {
                        v[idx]
                    };
                    return ((v[idx] - vm1) / self.dx, (vp1 - v[idx]) / self.dx);
                }
                (
            ...
            1 => {
                ...
                if j < 2 || j >= self.ny - 3 {
                    ...
                    let vp1 = if j < self.ny - 1 {
                        v[self.index(i, j + 1, k)]
                    } else {
                        v[idx]
                    };
                    return ((v[idx] - vm1) / self.dy, (vp1 - v[idx]) / self.dy);
                }
                (
            ...
            _ => {
                ...
                if k < 2 || k >= self.nz - 3 {
                    ...
                    let vp1 = if k < self.nz - 1 {
                        v[self.index(i, j, k + 1)]
                    } else {
                        v[idx]
                    };
                    return ((v[idx] - vm1) / self.dz, (vp1 - v[idx]) / self.dz);
                }
                (
        ...
        let beta0 = (thirteen * (vm2 - two * vm1 + v0).powi(2)
            + three
                * (vm2 - T::from_f64(4.0).unwrap_or_else(|| T::zero()) * vm1 + three * v0).powi(2))
            / T::from_f64(12.0).unwrap_or_else(|| T::zero());
        let beta1 = (thirteen * (vm1 - two * v0 + vp1).powi(2) + three * (vm1 - vp1).powi(2))
            / T::from_f64(12.0).unwrap_or_else(|| T::zero());
        let beta2 = (thirteen * (v0 - two * vp1 + vp2).powi(2)
            + three
                * (three * v0 - T::from_f64(4.0).unwrap_or_else(|| T::zero()) * vp1 + vp2).powi(2))
            / T::from_f64(12.0).unwrap_or_else(|| T::zero());
        ...
        let derivative_minus = (w0
            * (two * vm1 - T::from_f64(7.0).unwrap_or_else(|| T::zero()) * v0
                + T::from_f64(11.0).unwrap_or_else(|| T::zero()) * vp1)
            + w1 * (-vm1 + T::from_f64(5.0).unwrap_or_else(|| T::zero()) * v0 + two * vp1)
            + w2 * (two * v0 + T::from_f64(5.0).unwrap_or_else(|| T::zero()) * vp1 - vp2))
            / (six * h);

        // Positive-biased stencil (for negative velocities)
        // Need to recalculate with shifted stencil
        let derivative_plus = (w2
            * (T::from_f64(11.0).unwrap_or_else(|| T::zero()) * v0
                - T::from_f64(7.0).unwrap_or_else(|| T::zero()) * vp1
                + two * vp2)
            + w1 * (two * vm1 + T::from_f64(5.0).unwrap_or_else(|| T::zero()) * v0 - vp1)
            + w0 * (-vm2 + T::from_f64(5.0).unwrap_or_else(|| T::zero()) * vm1 + two * v0))
            / (six * h);
    ...
    pub fn advect(&mut self, dt: T) {
        ...
        if cfl > T::from_f64(self.config.cfl_number).unwrap_or_else(|| T::zero()) {
            ...
            let recommended_dt = T::from_f64(self.config.cfl_number).unwrap_or_else(|| T::zero())
                * min_spacing
                / max_velocity;
            eprintln!(
                "Warning: CFL = {:?} > {:?}, recommended dt = {:?}",
            ...
            for k in 0..self.nz {
                for j in 0..self.ny {
                    for i in 0..self.nx {
                        ...
                        let (dphi_dx_minus, dphi_dx_plus) =
                            self.weno5_derivative(&self.phi_previous, 0, i, j, k);
                        let (dphi_dy_minus, dphi_dy_plus) =
                            self.weno5_derivative(&self.phi_previous, 1, i, j, k);
                        let (dphi_dz_minus, dphi_dz_plus) =
                            self.weno5_derivative(&self.phi_previous, 2, i, j, k);
                        ...
                        self.phi[idx] = self.phi_previous[idx]
                            - dt * (vel[0] * dphi_dx + vel[1] * dphi_dy + vel[2] * dphi_dz);
                    }
                }
            }
        } else {
            ...
            for k in 1..self.nz - 1 {
                for j in 1..self.ny - 1 {
                    for i in 1..self.nx - 1 {
                        ...
                        // Upwind differences
                        let dphi_dx = if vel[0] > T::zero() {
                            (self.phi_previous[idx] - self.phi_previous[self.index(i - 1, j, k)])
                                / self.dx
                        } else {
                            (self.phi_previous[self.index(i + 1, j, k)] - self.phi_previous[idx])
                                / self.dx
                        };

                        let dphi_dy = if vel[1] > T::zero() {
                            (self.phi_previous[idx] - self.phi_previous[self.index(i, j - 1, k)])
                                / self.dy
                        } else {
                            (self.phi_previous[self.index(i, j + 1, k)] - self.phi_previous[idx])
                                / self.dy
                        };

                        let dphi_dz = if vel[2] > T::zero() {
                            (self.phi_previous[idx] - self.phi_previous[self.index(i, j, k - 1)])
                                / self.dz
                        } else {
                            (self.phi_previous[self.index(i, j, k + 1)] - self.phi_previous[idx])
                                / self.dz
                        };
    ...
    fn smooth_heaviside(&self, phi: T, epsilon: T) -> T {
        if phi < -epsilon {
            ...
            half * (T::one() + phi / epsilon + (pi * phi / epsilon).sin() / pi)
        }
    }
        ...
        for k in 1..self.nz - 1 {
            for j in 1..self.ny - 1 {
                for i in 1..self.nx - 1 {
                    ...
                    let phi_x = (self.phi[self.index(i + 1, j, k)]
                        - self.phi[self.index(i - 1, j, k)])
                        / (T::from_f64(2.0).unwrap_or_else(|| T::zero()) * self.dx);
                    let phi_y = (self.phi[self.index(i, j + 1, k)]
                        - self.phi[self.index(i, j - 1, k)])
                        / (T::from_f64(2.0).unwrap_or_else(|| T::zero()) * self.dy);
                    let phi_z = (self.phi[self.index(i, j, k + 1)]
                        - self.phi[self.index(i, j, k - 1)])
                        / (T::from_f64(2.0).unwrap_or_else(|| T::zero()) * self.dz);

                    // Second derivatives
                    let phi_xx = (self.phi[self.index(i + 1, j, k)]
                        - T::from_f64(2.0).unwrap_or_else(|| T::zero()) * self.phi[idx]
                        + self.phi[self.index(i - 1, j, k)])
                        / (self.dx * self.dx);
                    let phi_yy = (self.phi[self.index(i, j + 1, k)]
                        - T::from_f64(2.0).unwrap_or_else(|| T::zero()) * self.phi[idx]
                        + self.phi[self.index(i, j - 1, k)])
                        / (self.dy * self.dy);
                    let phi_zz = (self.phi[self.index(i, j, k + 1)]
                        - T::from_f64(2.0).unwrap_or_else(|| T::zero()) * self.phi[idx]
                        + self.phi[self.index(i, j, k - 1)])
                        / (self.dz * self.dz);

                    // Mixed derivatives
                    let phi_xy = (self.phi[self.index(i + 1, j + 1, k)]
                        - self.phi[self.index(i + 1, j - 1, k)]
                        - self.phi[self.index(i - 1, j + 1, k)]
                        + self.phi[self.index(i - 1, j - 1, k)])
                        / (T::from_f64(4.0).unwrap_or_else(|| T::zero()) * self.dx * self.dy);
                    let phi_xz = (self.phi[self.index(i + 1, j, k + 1)]
                        - self.phi[self.index(i + 1, j, k - 1)]
                        - self.phi[self.index(i - 1, j, k + 1)]
                        + self.phi[self.index(i - 1, j, k - 1)])
                        / (T::from_f64(4.0).unwrap_or_else(|| T::zero()) * self.dx * self.dz);
                    let phi_yz = (self.phi[self.index(i, j + 1, k + 1)]
                        - self.phi[self.index(i, j + 1, k - 1)]
                        - self.phi[self.index(i, j - 1, k + 1)]
                        + self.phi[self.index(i, j - 1, k - 1)])
                    ...
                    let grad_norm = ComplexField::sqrt(
                        grad_norm_sq + T::from_f64(1e-10).unwrap_or_else(|| T::zero()),
                    );
                ...
                if *p > eps {
                    ...
                    T::from_f64(0.5).unwrap_or_else(|| T::zero())
                        * (T::one()
                            - *p / eps
                            - ComplexField::sin(
                                T::from_f64(std::f64::consts::PI).unwrap_or_else(|| T::zero()) * *p
                                    / eps,
    ...
    pub fn step(&mut self, dt: T) -> Result<()> {
        ...
        let dt_cfl = T::from_f64(self.config.cfl_number).unwrap_or_else(|| T::zero())
            * self.dx.min(self.dy).min(self.dz)
            / (max_vel + T::from_f64(1e-10).unwrap_or_else(|| T::zero()));
...
mod tests {
    ...
    fn test_level_set_creation() {
        ...
        let solver: LevelSetSolver<f64> = LevelSetSolver::new(config, 10, 10, 10, 0.1, 0.1, 0.1);
    ...
    fn test_sphere_initialization() {
        let config = LevelSetConfig::default();
        let mut solver: LevelSetSolver<f64> =
            LevelSetSolver::new(config, 20, 20, 20, 0.1, 0.1, 0.1);
    ...
    fn test_volume_fraction() {
        let config = LevelSetConfig::default();
        let mut solver: LevelSetSolver<f64> =
            LevelSetSolver::new(config, 10, 10, 10, 0.1, 0.1, 0.1);

...
impl<T: RealField + FromPrimitive + Copy> IbmSolver<T> {
    ...
    pub fn new(config: IbmConfig, nx: usize, ny: usize, nz: usize, dx: T, dy: T, dz: T) -> Self {
    ...
    fn delta_function(&self, r: T) -> T {
        ...
        let r_norm = r.abs() / h;
        if r_norm >= cfd_core::numeric::from_f64(2.0)? {
            T::zero()
        } else if r_norm < T::one() {
            let one = T::one();
            let three = cfd_core::numeric::from_f64(3.0)?;
                ...
                for dj in 0..INTERPOLATION_STENCIL_SIZE {
                    for dk in 0..INTERPOLATION_STENCIL_SIZE {
                        ...
                        let grid_pos = Vector3::new(
                            cfd_core::numeric::from_usize(i)? * dx,
                            cfd_core::numeric::from_usize(j)? * dy,
                            cfd_core::numeric::from_usize(k)? * dz,
                        );
                        ...
                        let weight = weight_x * weight_y * weight_z * dx * dy * dz;
    ...
    pub fn calculate_forces(&mut self, target_velocity: Option<&[Vector3<T>]>) {
        if self.config.use_direct_forcing {
            ...
            for (i, point) in self.lagrangian_points.iter_mut().enumerate() {
                ...
                point.force = (u_target - point.velocity)
                    * cfd_core::numeric::from_f64(self.config.force_scale)?;
            ...
            for point in &mut self.lagrangian_points {
                if let Some(ref_pos) = &point.reference_position {
                    ...
                    let stiffness = cfd_core::numeric::from_f64(100.0)?; // Spring stiffness
    ...
    pub fn spread_forces(&mut self) {
        ...
        for point in &self.lagrangian_points {
            ...
            // Spread force using delta function
                        let i = (i_center + di)
                            .saturating_sub(stencil_half)
                            .min(self.nx - 1);
                        let j = (j_center + dj)
                            .min(self.ny - 1);
                        let k = (k_center + dk)
                            .min(self.nz - 1);
                            cfd_core::numeric::from_usize(i)? * self.dx,
                            cfd_core::numeric::from_usize(j)? * self.dy,
                            cfd_core::numeric::from_usize(k)? * self.dz,
...
mod tests {
    ...
    fn test_force_spreading() {
        let mut solver: IbmSolver<f64> = IbmSolver::new(config, 10, 10, 10, 1.0, 1.0, 1.0);

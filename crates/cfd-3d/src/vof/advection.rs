...
use cfd_core::error::{Result, Error, NumericalErrorKind};
...
impl AdvectionMethod {
    ...
    fn calculate_flux<T: RealField + FromPrimitive + Copy>(
        ...
        let velocity =
            (solver.velocity[idx1] + solver.velocity[idx2]) * T::from_f64(0.5).ok_or_else(|| Error::Numerical(NumericalErrorKind::ConversionFailed { from_type: "f64", to_type: std::any::type_name::<T>(), value: "value".to_string() }))?;
        ...
        for k in 1..solver.nz - 1 {
            for j in 1..solver.ny - 1 {
                for i in 1..solver.nx - 1 {
                    ...
                    if alpha > T::from_f64(VOF_INTERFACE_LOWER).ok_or_else(|| Error::Numerical(NumericalErrorKind::ConversionFailed { from_type: "f64", to_type: std::any::type_name::<T>(), value: "value".to_string() }))?
                        && alpha < T::from_f64(VOF_INTERFACE_UPPER).unwrap_or(T::one())
                    {
                        ...
                        if normal.norm() > T::zero() {
                            ...
                            let compression_factor = T::from_f64(0.5).ok_or_else(|| Error::Numerical(NumericalErrorKind::ConversionFailed { from_type: "f64", to_type: std::any::type_name::<T>(), value: "value".to_string() }))?;

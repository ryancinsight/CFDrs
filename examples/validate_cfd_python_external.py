#!/usr/bin/env python3
"""
cfd_python External CFD Package Validation Suite

This script validates cfd_python (Rust CFD via PyO3) against:
...
Usage:
    python validate_cfd_python_external.py

Requirements:
    - cfd_python (built with `maturin develop`)
...
try:
    ...
    print("Error: cfd_python not found. Build with: maturin develop")
    sys.exit(1)
...
def test_poiseuille_2d() -> ValidationResult:
    ...
    # Solve using cfd_python
    solver = cfd_python.Poiseuille2DSolver(height, height, length, nx, ny)
...
def test_bifurcation_murray() -> ValidationResult:
    ...
    # Solve using cfd_python
    solver = cfd_python.BifurcationSolver(d_parent, d_daughter1, d_daughter2)
...
def test_trifurcation() -> ValidationResult:
    ...
    # Solve using cfd_python
    solver = cfd_python.TrifurcationSolver(d_parent, d_daughter, d_daughter, d_daughter)
    result = solver.solve(flow_rate, pressure, "casson")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("cfd_python External CFD Package Validation Suite")
    print("=" * 70)
    print("\nValidating cfd_python against:")
    ...
    with open("cfd_python_validation_report.json", "w") as f:
        json.dump(report, f, indent=2)
    
    print(f"\nReport saved to: cfd_python_validation_report.json")
    
    # Final status
    if passed_count == len(results):
        print("\n🎉 ALL VALIDATIONS PASSED!")
        print("   cfd_python produces results consistent with:")

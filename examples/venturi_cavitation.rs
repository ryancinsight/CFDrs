...
fn generate_venturi_geometry() -> Result<cfd_mesh::Mesh<f64>, Box<dyn std::error::Error>> {
    ...
    let venturi = CsgBuilder::new()
        ...
        .add({
            let mut throat = operator.create_cylinder(
                dim::THROAT_DIAMETER / 2.0,
                dim::THROAT_LENGTH,
                32
            )?;
            throat.translate(&Vector3::new(0.0, 0.0, dim::INLET_LENGTH + dim::CONVERGENT_LENGTH))?;
            throat
        })?
        ...
        .add({
            let mut outlet = operator.create_cylinder(
                dim::OUTLET_DIAMETER / 2.0,
                dim::OUTLET_LENGTH,
                32
            )?;
            outlet.translate(&Vector3::new(
                0.0, 0.0,
                dim::INLET_LENGTH + dim::CONVERGENT_LENGTH + dim::THROAT_LENGTH + dim::DIVERGENT_LENGTH
            ))?;
            outlet
        })?
        .build()?;
    ...
    fs::write("output/venturi_cavitation/venturi_geometry.stl", stl_content)?;
    
    // Get geometry statistics
    let (min_bounds, max_bounds) = venturi.bounding_box()?;
    println!("   📐 Venturi dimensions:");
    println!("      Total length: {:.1} mm", (max_bounds.z - min_bounds.z) * 1000.0);
    println!("      Inlet diameter: {:.1} mm", dim::INLET_DIAMETER * 1000.0);
    println!("      Throat diameter: {:.1} mm", dim::THROAT_DIAMETER * 1000.0);
    println!("      Outlet diameter: {:.1} mm", dim::OUTLET_DIAMETER * 1000.0);
    println!("      Contraction ratio: {:.2}", 
             (dim::INLET_DIAMETER / dim::THROAT_DIAMETER).powi(2));
    println!("   ✓ Geometry generated: {} vertices, {} faces", 
...
fn analyze_1d_cavitation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   📍 Cavitation inception velocity: {:.2} m/s", v_inception);
...
fn simulate_2d_cavitation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let total_length = dim::INLET_LENGTH + dim::CONVERGENT_LENGTH + 
                      dim::THROAT_LENGTH + dim::DIVERGENT_LENGTH + dim::OUTLET_LENGTH;
    ...
    for i in 0..grid.nx() {
        ...
        boundary_conditions.insert(
            (i, 0),
            BoundaryCondition::Wall { wall_type: WallType::Slip }, // Symmetry
        );
...
fn simulate_3d_cavitation(mesh: cfd_mesh::Mesh<f64>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for step in 0..n_steps {
        ...
        if step % 10 == 0 {
            let avg_radius: f64 = bubbles.iter().map(|b| b.radius).sum::<f64>() / bubbles.len() as f64;
            println!("   Step {}: avg bubble radius = {:.2} μm", 
                     step, avg_radius * 1e6);
        }
    }
    ...
    println!("   ⏱️ Rayleigh collapse time: {:.2} μs", collapse_time * 1e6);
...
mod tests {
    ...
    fn test_venturi_radius_calculation() {
        ...
        assert_eq!(calculate_venturi_radius(dim::INLET_LENGTH + dim::CONVERGENT_LENGTH + dim::THROAT_LENGTH / 2.0), 
                   dim::THROAT_DIAMETER / 2.0);
    }

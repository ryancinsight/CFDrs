...
fn flow_around_csg_cylinder() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        Ok(_) => {
            ...
            println!(
                "   📊 Solution extracted: {} pressure nodes, {} velocity nodes",
                pressure.len(),
                velocity.len()
            );
...
fn flow_through_csg_pipe() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   📐 Pipe bounds: ({:.2}, {:.2}, {:.2}) to ({:.2}, {:.2}, {:.2})",
        min_bounds.x, min_bounds.y, min_bounds.z, max_bounds.x, max_bounds.y, max_bounds.z
    );
    println!(
        "   📊 Pipe mesh: {} vertices, {} faces",
        pipe_geometry.vertex_count(),
        pipe_geometry.face_count()
    );

    // Convert to CFD mesh for 3D FEM solver
    let cfd_mesh = pipe_geometry.to_mesh()?;
    println!(
        "   🔄 Converted to CFD mesh: {} vertices, {} faces",
        cfd_mesh.vertices.len(),
        cfd_mesh.faces.len()
    );
    ...
    println!(
        "      Fluid: Water (ρ={:.1} kg/m³, μ={:.1e} Pa·s)",
        fluid_density, fluid_viscosity
    );
    println!(
        "      Reynolds number: {:.0}",
        fem_config.reynolds.unwrap_or(0.0)
    );
...
fn heat_transfer_csg_fin() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let volume = (max_bounds.x - min_bounds.x)
    ...
    println!(
        "      Estimated surface area: {:.3} m²",
        surface_area_estimate
    );
    println!(
        "      Surface-to-volume ratio: {:.1}",
        surface_area_estimate / volume
    );
...
fn complex_csg_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "      CSG mesh: {} vertices, {} faces",
        vertex_count, face_count
    );
    println!(
        "      CFD mesh: {} vertices, {} faces",
        cfd_mesh.vertices.len(),
        cfd_mesh.faces.len()
    );
    ...
    // Mesh quality checks
    let aspect_ratio =
        (max_bounds.x - min_bounds.x) / (min_bounds.y - max_bounds.y).abs().max(1e-6);

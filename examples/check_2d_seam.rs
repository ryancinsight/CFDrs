...
fn check_mesh(name: &str, mesh: &cfd_mesh::IndexedMesh) {
    ...
    println!("{}: faces={} boundary_edges={} non_manifold_edges={}", 
             name, mesh.faces.len(), report.boundary_edges, report.non_manifold_edges);
    ...
    if report.non_manifold_edges > 0 {
        println!("  WARNING: {} has {} non-manifold edges (duplicate/overlap?)", name, report.non_manifold_edges);
    }
}

...
use cfd_3d::bifurcation::{
    types::BifurcationSolution3D, BifurcationConfig3D, BifurcationGeometry3D, BifurcationSolver3D,
};
use cfd_core::physics::fluid::blood::CassonBlood;
use cfd_core::physics::fluid::{ConstantPropertyFluid, FluidTrait, NonNewtonianFluid};
...
fn test_laminar_flow(
    solver: &BifurcationSolver3D<f64>,
    fluid: &CassonBlood<f64>,
) -> ValidationResult {
...
fn validate_symmetric_bifurcation() -> Vec<ValidationResult> {
    ...
    println!(
        "  Daughter diameter: {:.1} μm (Murray's law)",
        d_daughter * 1e6
    );
    ...
    // Create solver
    let geometry = BifurcationGeometry3D::symmetric(d_parent, d_daughter, length, length, 100e-6);
    ...
    println!(
        "  Yield stress: {:.4} Pa",
        blood.yield_stress().unwrap_or(0.0)
    );
    ...
    results.push(test_poiseuille_wss(
        &solution,
        blood_props.dynamic_viscosity,
        d_parent,
        0.10,
    ));
...
fn validate_asymmetric_bifurcation() -> Vec<ValidationResult> {
    ...
    println!("  Parent diameter: {:.1} μm", d_parent * 1e6);
    println!(
        "  Daughter 1 diameter: {:.1} μm (smaller)",
        d_daughter1 * 1e6
    );
    println!(
        "  Daughter 2 diameter: {:.1} μm (larger)",
        d_daughter2 * 1e6
    );

    // Check Murray's law deviation
    let murray_deviation =
        (d_parent.powi(3) - (d_daughter1.powi(3) + d_daughter2.powi(3))).abs() / d_parent.powi(3);
...
fn main() {
    ...
    // Combine results
    let all_results: Vec<_> = symmetric_results
        .into_iter()

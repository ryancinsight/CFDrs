...
fn test_4_pressure_recovery() {
    ...
    let p_outlet_actual: f64 =
        p_inlet + eta_diffuser * 0.5 * rho * (v_inlet.powi(2) - v_outlet.powi(2));
    ...
    println!("    p_inlet = {:.0} Pa ({:.2} bar)", p_inlet, p_inlet / 1e5);
    println!(
        "    p_outlet (ideal) = {:.0} Pa ({:.2} bar)",
        p_outlet_ideal,
        p_outlet_ideal / 1e5
    );
    println!(
        "    p_outlet (actual, η=80%) = {:.0} Pa ({:.2} bar)",
        p_outlet_actual,
        p_outlet_actual / 1e5
    );
...
fn test_5_incipient_cavitation() {
    ...
    println!(
        "    Incipient cavitation number σᵢ = {:.2} ± {:.2}",
    ...
    println!("  Results:");
    println!(
        "    Inception velocity V_inception ≈ {:.1} m/s",
        v_inception
    );
    println!(
        "    At this velocity: σ ≈ {:.2} (target: {:.2})",
    ...
    if beta_error < 0.01 {
        ...
        println!(
            "  ⚠️  NOTE: Geometry differs from Rudolf et al. (β = {:.2} vs {:.2})",

...
- **Evidence:** the pending records now sort in place with no `to_vec()` clone; the ordering regression and existing routing geometry/collision tests pass in 214/214 native tests. Warning-denied Clippy, 16/16 doctests, rustfmt, diff checks, and standalone lockfile validation pass.
...
| ID | Outcome | Class | Status | Owner | Scope |
|----|---------|-------|--------|-------|-------|
| CFDRS-SCHEMATICS-CURVATURE-SCAN | Detect visible serpentine curvature directly from path offsets without allocating an intermediate vector. | [perf] | IMPLEMENTED | Codex | `crates/cfd-schematics/src/visualizations/schematic/path_generation.rs`, path-generation tests |
...
- **Verification:** focused package compile, native Nextest, warning-denied Clippy, doctests, rustfmt/diff checks, and standalone lockfile validation.
- **Evidence:** `cfd-schematics` compilation, 210/210 Nextest tests, warning-denied Clippy, 16/16 doctests, rustfmt, diff checks, and standalone lockfile validation pass. The focused regressions cover both-sided curvature, one-sided rejection, and threshold rejection; source review confirms the scan is lazy and exits as soon as both signs are observed.
...
## CFDRS-SCHEMATICS-VOLUME-SUMMARY-STREAM — stream aggregate volume calculation [perf] — IMPLEMENTED 2026-09-03

| ID | Outcome | Class | Status | Owner | Scope |
|----|---------|-------|--------|-------|-------|
| CFDRS-SCHEMATICS-VOLUME-SUMMARY-STREAM | Compute aggregate fluid-volume totals directly from channel geometry without materializing per-channel summaries or identifier strings. | [perf] | IMPLEMENTED | Codex | `crates/cfd-schematics/src/geometry/types/volume.rs`, volume tests |
...
- **Verification:** focused package compile, native Nextest, warning-denied Clippy, doctests, rustfmt/diff checks, and standalone lockfile validation.
- **Evidence:** `cfd-schematics` compilation, 207/207 Nextest tests, warning-denied Clippy, 16/16 doctests, rustfmt, diff checks, and standalone lockfile validation pass. The two-channel volume regression compares aggregate totals and exact display-label formatting with per-channel reductions; source review confirms the aggregate path no longer constructs per-channel summaries or identifier strings.
...
- **Acceptance:** existing channel paths are borrowed during bounds, length,
  and routing analysis; only empty-path overlay inputs allocate; annotated
  geometry, lengths, roles, and venturi routing remain unchanged; focused
  package gates pass with no public API change.
- **Non-goals:** changing selective geometry formulas, path routing policy,
  generated coordinates, or peer-owned files.
- **Lease:** Ryan Clanton —
...
- **Evidence:** metadata-backed paths now borrow through resolution and render
  with unchanged values; 204/204 nextest tests, warning-denied Clippy, 16/16
  doctests, rustfmt, and standalone lockfile validation pass.
...
- **Acceptance:** complete channel paths remain borrowed during pairwise
  intersection scans; empty and singleton paths still synthesize owned
  endpoint centerlines; crossing counts, locations, and insertion behavior are
  unchanged; focused package gates pass with no public API change.
- **Non-goals:** changing segment-intersection tolerances, pairwise scan
  ordering, topology insertion semantics, or peer-owned files.
- **Lease:** Ryan Clanton —
...
- **Acceptance:** pending venturi routing checks previously assigned channel
  paths through borrowed slices without cloning their point buffers; direct,
  dogleg, and fallback routing preserve their existing geometry and collision
...
- **Local evidence (2026-08-21):** `maturin build --locked --release`
  produced an abi3-py38 wheel containing `cfd_python.pyi` and
  `cfd_python/py.typed`; installed-wheel pytest passes 4/4, strict mypy
  consumer validation passes, and runtime export coverage finds no public
  extension name missing from the stub. The selected bifurcation and legacy
  Poiseuille methods convert Python-owned values before `Python::detach` and
  perform Rust-only solver work outside the GIL. `cargo nextest` runs 0/0 for
  the cdylib package, and locked package compilation passes.
- **Residual:** package Clippy is blocked by the unrelated peer-dirty
  `cfd-2d/src/solvers/ns_fvm/field.rs:25` unused import; hosted exact-head
...
- **CFDRS-AEQ-MET-63 [major] - Type ChannelSpec hydraulic metrics
  ...
  Eunomia-backed Aequitas quantities. Valve `Cv` is represented through its
  ...
  serialization boundaries. Evidence: cfd-schematics and cfd-1d value-
  semantic tests, cfd-schematics/cfd-1d Nextest `de05acf2-aedc-4d80-ac95-
  2dba555b669f` (930/930, 3 skipped), affected all-target check, focused
  warning-denied Clippy, doctests (16/16 and 8/8 with 3 ignored), and Rustdoc.
  The real hydraulic contract has no complex or imaginary SI quantity.
...
- **CFDRS-AEQ-MET-61 [major] - Type TPMS lattice period metrics
  ...
  periods now carry Eunomia `Length<f64>` values in base metres, with scalar
  extraction only at mesh and rendering formula boundaries. Normalized
  fractions and iso-values remain dimensionless. Evidence: affected
  all-target check, warning-denied Clippy, cfd-schematics Nextest
  `2d46b8a3-990a-4caa-91e2-2e1051519d5c` (191/191), cfd-schematic-mesh
  Nextest `f8a94bb6-b0bf-462f-a27b-3beb50211c81` (29/29), 16/16 doctests,
  Rustdoc, and JSON value-semantic round-trip coverage. This real geometry
...
- **CFDRS-AEQ-MET-56 [major] - Type Milestone 12 request geometry (done
  ...
  request now stores plate dimensions, channel dimensions, branch/outlet
  lengths, and venturi geometry as Eunomia `Length<f64>` values in base metres.
  Split fractions and enum controls remain dimensionless. Direct consumers
  migrate without adapters; scalar extraction remains at layout, validation,
  and geometry boundaries. The typed request propagation regression and
  affected package gates pass. Complex or imaginary SI quantities are out of
  scope because this is real geometry. Evidence: all-target checks pass for
  cfd-schematics, cfd-optim, cfd-schematic-mesh, cfd-1d, and cfd-2d;
  warning-denied Clippy passes for cfd-schematics, cfd-optim, and
  cfd-schematic-mesh; Nextest passes cfd-schematics 189/189, cfd-optim
  137/137, and cfd-schematic-mesh 29/29; cfd-schematics doctests pass 16/16,
  cfd-optim doctests pass 2/2 with 3 ignored, and Rustdoc builds all three
...
- **CFDRS-AEQ-MET-50 [major] - Type channel route geometry (done 2026-08-05;
  owner=current Codex session; scope=`ChannelRouteSpec` route length, width,
  and height plus direct schematic/optimization callers).** The public route
  authoring contract now carries all three dimensions as Eunomia `Length<f64>`
  values in base metres. Direct constructors and consumers are migrated
  without adapters; scalar extraction remains at validation, routing, mesh,
  formula, reporting, and serialization boundaries. `SerpentineSpec`,
  `SubBranchSpec`, and `BlueprintTopologySpec` envelope fields remain separate
  audit boundaries. The builder/JSON round-trip regression preserves route
  values within the derived one-binary64-round-trip bound. Evidence: current
  all-targets checks pass; warning-denied Clippy passes for cfd-schematics and
  cfd-optim; cfd-schematics Nextest passes 183/183
  (`0fc16e9a-d734-43f0-a7d3-bb93fd9d187c`); cfd-optim Nextest passes 137/137
  across five binaries (`7747ec29-d04c-458a-a7fc-21fceee58f7d`); focused
  schematic export passes 7/7 (`da5aef30-c34e-492b-a416-118f167f690f`);
...
- **CFDRS-AEQ-MET-42 [major] - Consolidate legacy analytical benchmarks
  ...
  physical fields. The superseded implementations are removed, the direct
  consumer uses `cfd_validation::analytical`, and the unique lid-driven-cavity
  reference tables are preserved. The duplicate-model residue scan, typed
  consumer construction, targeted Rustfmt, and diff checks pass. The focused
  `physics_validation` Nextest remains blocked before cfd-validation by the
  peer-dirty cfd-math `leto-ops` API mismatch; this is an external verification
...
- **CFDRS-AEQ-MET-24 [major] - Type cavitation-VOF physical metrics
  ...
  relaxation time, pressure, mass density, and sound-speed values in the
  cavitation-VOF boundary. Carry them through Aequitas quantities, extract
  only at Rayleigh-Plesset, mesh, and dense-field formula boundaries, preserve
...
- **CFDRS-AEQ-MET-22 [major] - Type transient mixture fractions
  ...
  carried raw scalar dimensionless values. They now use Aequitas
  `Dimensionless<T>` and convert only inside mixture normalization, arithmetic,
  and numerical transport boundaries. Solver residual norms remain
  equation-dependent and are a non-goal. Acceptance is a public-signature
  residue audit, value-semantic mixture regressions, focused cfd-1d gates, and
  no compatibility facade. The test-target check passes; package Nextest passes
  736/736 with 3 skips; warning-denied all-target Clippy passes; and doctests
  pass 8/8 with 3 ignored.
...
- **CFDRS-AEQ-MET-21 [major] - Type transient composition timepoint vectors
  ...
  simulation entry points and generated timepoint accessors still expose
  `Vec<T>`. Carry requested, calculated, and returned timepoints as
  `Vec<Time<T>>`; convert only at sorting, tolerance, and solver boundaries.
...
- **CFDRS-AEQ-MET-18 [major] - Type network node and metadata metrics
  ...
  `cargo check -p cfd-1d --offline` passes; Nextest passes 735/735 with 3
  skips; doctests pass 8/8 with 3 ignored. The warning-denied Clippy result is
...
- **CFDRS-AEQ-MET-16 [major] - Type network hydraulic coefficient metrics
  ...
  coefficient/flow callers).** Edge flow, linear resistance, quadratic
  resistance, and derived conductance now use named Aequitas quantities at
  public edge boundaries; scalar extraction is confined to numerical kernels.
  Network pressure/flow/residual vectors remain a separately audited
  solver-state boundary. Locked `cargo check -p cfd-1d` passes; Nextest passes
  731/731 with 3 skipped; cfd-1d doctests pass 8/8 with 3 ignored. The
  rustdoc/link and lint baseline notes remain outside this metric slice.
...
- **CFDRS-GA-015 [patch][arch] — Restore the pedantic floor erased by crate-level allows (status=in-progress, integrator=current session; lease: slice-2 files below).**
  Count correction 2026-09-18: the 96/97 figure is outer `#[allow(` only; inner `#![allow(` (mostly `unwrap_used`+`missing_docs` in test files, the sanctioned test-exemption shape) adds 222. True allow-sites = 318 outer+inner. Slice 2 scope (production + test-support, no design-lint refactors): the 3 `print_stderr` allows → `expect(reason="test/validation output")` per the sibling pattern, `manual_clamp` → `clamp`, `items_after_statements` const hoist, test-helper by-value, 2 dead `_` bindings removed, fem-test `field_reassign` → struct-update. Deferred: `too_many_arguments` ×27 (needs param bundling), `needless_range_loop` ×2 (numerical loop-form risk), `cfd-optim` 336 docs, redundant workspace-covered outers (separate cleanup slice).
  ...
  Slice 2 evidence: 8 outer + 2 inner allows removed across 9 files — `manual_clamp`→`clamp` (value-identical incl. NaN), `items_after_statements` const hoist, test-helper by-value + 15 call sites, 2 dead `_` bindings deleted with context preserved in comments, fem-test `field_reassign`→struct-update ×5, 1 dead `print_stderr` suppression deleted outright (no print macros in the fn — verified by Select-String, not grep), 3 live test-diagnostic suppressions migrated to `expect(reason=...)` per the sibling pattern. Counts: outer 96→88, inner 222→220 (total 318→308). Clippy `-D warnings` clean on all 6 affected packages; affected suites green (backend_validation 13/13, fem 7/7, momentum 2/2, cylinder 7/7, cell_separation 155/155, figures 8/8, render_core 1/1). Two process notes: standalone gates must run from outside the stack root with `CARGO_TARGET_DIR=D:\atlas\target` (else the overlay breaks `--locked` and forks `target/` — the forked `crates/.../target` tree was deleted); plain `grep` silently missed matches that Select-String and clippy found, so compiler-oracle verification is mandatory after every suppression edit.
  Slice 1 evidence: all 76 `cfd-python` warns resolved with unit-carrying field docs (verified against solver bodies and `__str__` unit displays); `cargo doc -p cfd-python` emits 0 warnings, fmt/diff-check clean, `clippy -p cfd-python --all-targets -- -D warnings` clean. Crate-level 2→1, per-site steady at 96. No API change (docs-only), so the `.pyi` stub needs no sync. Landed as PR #432 (`552448db`); lease released. Next slice: `cfd-optim` per-module docs (336 warns) then the 96 per-site allows.

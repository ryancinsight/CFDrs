...
- [ ] Use the precomputed node index in `NetworkBlueprint::to_interchange` for empty-path endpoint geometry, replacing repeated node scans with constant-time lookups while preserving the existing fallback and endpoint-id contract; add a non-ordered-node value-semantic regression. Status: in-progress; owner: current Codex session; lease: `crates/cfd-schematics/src/geometry/types/interchange.rs`.

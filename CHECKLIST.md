...
- [ ] Move the owned position and auto-layout-index vectors out of `BlueprintNodeLayout` during `materialize_blueprint_layout` instead of cloning them across the borrow boundary; preserve auto-position and generated-path values with the existing regression. Status: in-progress; owner: current Codex session; lease: `crates/cfd-schematics/src/visualizations/schematic/{mod.rs,layout.rs}`.

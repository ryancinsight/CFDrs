...
- [ ] Use construction-order node indices in `create_series_geometry_from_spec` to replace two linear endpoint scans per channel with constant-time access while preserving endpoint IDs and generated paths; add a multi-channel value-semantic regression. Status: in-progress; owner: current Codex session; lease: `crates/cfd-schematics/src/geometry/generator/linear.rs`.

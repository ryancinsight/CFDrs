...
## CFDRS-SCHEMATICS-CURVATURE-SCAN [perf] — in progress

- [ ] Detect visible serpentine curvature in one pass without collecting
      qualifying offsets into a temporary vector.
- [ ] Preserve both-sided, one-sided, and thresholded curvature decisions with
...
- [x] Preserve per-channel summaries and aggregate values, units, and labels
...
- [x] Route explicit annotation references through the Plotters renderer without
      cloning `RenderConfig` or `SchematicAnnotations`.
- [x] Preserve annotated and non-annotated output, validation, and renderer
...
- [x] Preserve crossing coordinates/counts and intersection-node insertion with
...
- [x] Type fraction storage, constructors, blends, tolerances, and public
      ...
      boundaries.
- [x] Add value-semantic fraction/hematocrit/concentration regressions and run
      focused cfd-1d gates. The test-target check passes; package Nextest passes
      736/736 with 3 skips; warning-denied all-target Clippy passes; and
      doctests pass 8/8 with 3 ignored.
- [x] Synchronize the design note, gap audit, backlog, checklist, and
...
- [x] Type public requested, calculated, and returned timepoint vectors as
...
- [x] Add value-semantic timepoint regressions and run focused cfd-1d gates.
      The test-target check passes; package Nextest passes 736/736 with 3
      skips; warning-denied all-target Clippy passes; and the transient
      composition, droplet, and literature tests remain included in that
      package gate.
- [x] Synchronize the design note, gap audit, backlog, checklist, and
      changelog. Mixture fraction storage remains a separate dimensionless
      representation item.
...
- [x] Confirm the live droplet public contracts expose volume, time, and
      ...
      transient droplet scope.
- [x] Type droplet volume/time/fraction fields with Aequitas and migrate all
      simulator, constructor, and test callers without a compatibility facade.
...
## Owner: Codex — CFDRS-AEQ-MET-20 transient composition control metrics [major] — verified 2026-07-27

- [x] Confirm the live composition events, timing/control configuration, and
      ...
      scope.
- [x] Type the public temporal/control contracts with Aequitas and migrate all
...
- [x] Add value-semantic time, flow, pressure, hematocrit, and CFL regressions;
      run focused cfd-1d checks and synchronize the design note, gap audit,
      backlog, checklist, and changelog. The composition parity lane passes
      21/21; droplet parity passes 9/9; literature validation passes 5/5.
...
- [x] Run the locked cfd-1d check and focused Nextest; check passes and
      Nextest passes 731/731 with 3 skipped. The cfd-1d doctests pass 8/8 with
      3 ignored. Preserve peer-owned dirty solver files; the remaining
      rustdoc/link and lint baseline notes are outside this metric slice.
...
- [x] Type `ChannelType::Curved` radius and `Micromixer` hydraulic
      diameter/length with Aequitas `Length` values.
- [x] Keep scalar extraction at resistance and dynamic-parameter boundaries;
...
- [x] Keep scalar conversion at the resistance and trigonometric formula
...
- [x] Run the locked focused package checks and Nextest gates; record the
      non-Newtonian timeout and pre-existing Clippy debt exactly.
...
- [x] Reduce annotation lane path cloning in `cfd-schematics` by borrowing single forward segments as `Cow`, owning reversed segments, and materializing only merged lanes; verified with touched-file rustfmt/diff checks, `cargo check -p cfd-schematics --locked`, `cargo nextest run -p cfd-schematics --locked --no-fail-fast --status-level fail` (198/198), `cargo clippy -p cfd-schematics --all-targets --locked -- -D warnings`, `cargo test --doc -p cfd-schematics --locked` (16/16), and forward/reversed/merged lane value-semantic regressions.
- [x] Extract annotation lane assembly into a dedicated `visualizations/annotations/lane_paths.rs` leaf module so graph roles, markers, and lane geometry retain separate responsibilities; `geometry.rs` is 433 lines and `lane_paths.rs` is 135 lines. Verified with touched-file rustfmt/diff checks, `cargo check -p cfd-schematics --locked`, `cargo nextest run -p cfd-schematics --locked --no-fail-fast --status-level fail` (198/198), `cargo clippy -p cfd-schematics --all-targets --locked -- -D warnings`, and `cargo test --doc -p cfd-schematics --locked` (16/16).
- [x] Use the precomputed node index in `NetworkBlueprint::to_interchange` for empty-path endpoint geometry, replacing repeated node scans with constant-time lookups while preserving the existing fallback and endpoint-id contract; add a non-ordered-node value-semantic regression. Verified with the focused compile, native-test, all-target warning-denied Clippy, doctest, and temporary Criterion comparison harness. Same-overlay medians fell from 11.921/66.435/870.10 µs to 6.244/23.606/83.360 µs at 64/256/1024 nodes.
- [x] Use construction-order node indices in `create_series_geometry_from_spec` to replace two linear endpoint scans per channel with constant-time access while preserving endpoint IDs and generated paths; add a multi-channel value-semantic regression. Verified with overlay package Nextest (200/200), all-target warning-denied Clippy, 16 doctests, rustfmt/diff checks, and the standalone lockfile check. Same-overlay Criterion medians were 37.196/180.38/1.5832 ms before versus 36.102/177.36/1.5741 ms after at 64/256/1024 channels; the larger workloads showed no statistically significant end-to-end change, so this is recorded as a bounded complexity improvement rather than a broad throughput claim.
- [x] Move the owned position and auto-layout-index vectors out of `BlueprintNodeLayout` during `materialize_blueprint_layout` instead of cloning them across the borrow boundary; preserve auto-position and generated-path values with the existing regression. Verified with overlay package Nextest (200/200), all-target warning-denied Clippy, 16 doctests, rustfmt/diff checks, and the standalone lockfile check; source review confirms both layout-vector `to_vec` copies are removed. A temporary same-overlay allocator comparison was inconclusive because the shared target reused the benchmark dependency artifact, so it is not used as quantitative evidence.

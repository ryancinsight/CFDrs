...
def straight_pipe_dp_rect(mu, u, L, w, h):
    """Approximate ΔP for rectangular channel (Poiseuille, low-Re):
...
def validate_1d_serpentine():
    ...
    print("  [PASS] Positive ΔP")

    # Resistance
    R = result.resistance
    print(f"  Resistance: {R:.3e} Pa·s/m³")
...
def validate_1d_serpentine_scaling():
    ...
    for n in [5, 10, 20]:
        ...
        print(f"  n={n:3d}  ΔP={r.pressure_drop:.4f} Pa")

    # Check ~linear scaling: dp(20)/dp(10) ≈ 2
    ratio = dps[2][1] / dps[1][1]
    print(f"  ΔP ratio (20/10): {ratio:.2f}  (expected ~2.0)")
...
def validate_1d_serpentine_blood():
    ...
    for bt in ("newtonian", "casson", "carreau_yasuda"):
        try:
            ...
            results[bt] = r
            print(f"  {bt:20s}  ΔP={r.pressure_drop:10.4f} Pa  "
                  f"μ_app={r.apparent_viscosity:.6f} Pa·s  De={r.dean_number:.4f}")
    ...
    if "casson" in results and "newtonian" in results:
        ...
        print(f"  Casson/Newtonian ΔP ratio: {ratio:.3f}")
        assert ratio > 0.5, "Casson ΔP should be significant"
        print("  [PASS] Casson produces valid ΔP ratio")
...
def validate_3d_serpentine():
    ...
    try:
        ...
        print(f"  Total ΔP:       {result.dp_total:.4f} Pa")
...
def compare_with_straight_pipe():
    """Serpentine ΔP should exceed straight pipe of same total length."""
    print("=" * 60)
    print("Serpentine vs Straight Pipe ΔP Comparison")
    print("=" * 60)
    ...
    print(f"  Serpentine ΔP:     {result.pressure_drop:.4f} Pa")
    print(f"  Straight pipe ΔP:  {dp_straight:.4f} Pa")
    ...
    print("  [PASS] Serpentine ΔP is positive")

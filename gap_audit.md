...
| ID | Evidence | Closure |
|---|---|---|
| `CFDRS-AEQ-MET-14` | `cfd-1d` vascular `WomersleyNumber`, `WomersleyFlow`, `WomersleyProfile`, `VesselSegment`, `Bifurcation`, and `BifurcationNetwork` expose length, radius, pressure, density, viscosity, frequency, flow, and derived vascular results as raw scalar values. | **IN PROGRESS.** Aequitas now provides named pressure-gradient, hydraulic-resistance, hydraulic-inertance, and compliance dimensions in `446eb9f`. This increment types the public Womersley and bifurcation contracts and keeps scalar extraction only at analytical kernels. Murray/Olufsen radius APIs remain outside this bounded slice. |
...
MET-13 closes the first two; vascular/Womersley metrics remain the next audit
boundary. MET-14 now owns Womersley/bifurcation; Murray/Olufsen radius APIs
remain outside that slice and are not silently classified as closed.

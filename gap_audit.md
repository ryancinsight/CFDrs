...
| `CFDRS-AEQ-MET-19` | The transient droplet public boundary exposed droplet volume, injection/snapshot time, normalized positions, occupancy spans, and split thresholds as raw scalar values. | **IMPLEMENTED in this increment; focused verification pending peer repair.** Public and internal droplet state now carries Aequitas `Volume`, `Time`, and `Dimensionless` quantities, with scalar extraction confined to transport formulas. The same-revision `cargo check -p cfd-1d --tests --offline` passed before the peer graph shifted. Focused Nextest is blocked by the peer-owned cfd-math `apply_to` `LetoError`/`cfd_core::error::Error` mismatch; no droplet diagnostic was emitted. Composition event/time contracts remain a separately tracked residual. See [`transient-droplet-metrics.md`](docs/atlas-migration/transient-droplet-metrics.md). |
